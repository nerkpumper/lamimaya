...
// 	    $this->putText(10, "mita");  
	    ...
	    $this->setCurrentY($yInicial );
	    $this->putTextRight("ORDEN DE PRODUCCI�N", 20);
...
// 	    $this->putText(10, "Nota: Una vez confirmado el pedido no hay cambios ni devoluciones.");
	    ...
	    if ($this->__recogeEntrega == "ENTREGA")
	    {
			...
			// $this->RotatedText(60,175, $this->__rangoCliente,45);
...
//   		    }
            ...
            if ($index >= $rowInicial && $index < $rowFinal)
            {
                if ($item["detIdProducto"] != "")
                {
                    ...
                    $this->putTextRight($item["colocaCantidad"], 150);
                    ...
                    $desc = $item["proTipoProducto"] . " " .
                        ...
                        if ($item["proShortUnidad"] != 'PZA' && $item["proShortUnidad"] != 'KG' )
                        {
                            if ($item["proShortUnidad"] == 'ML')
                            {
								...
								elseif($item["proAplicacion"] == 'PANEL'){
									$desc .= " " .$item["proMedidaEspecial"] . " PULGADAS ".$item["detCantidad"]. " METRO LINEAL";
								}else 
								{
									$desc .=" ". $item["rolloPies"]. "  ". ( $item["idColor"] > 1 ? $item["color"] : "")  ." de ".$item["detCantidad"] . " " . $item["proShortUnidad"];								
								}
                        ...
                        {
                            if (floatval($item["proIdRollo"]) > 1)
                            {
                                $this->putTextRight(number_format(($item["detExplotarUnidad"]*$item["colocaCantidad"]),2) . " KG", 14.5);
                                $this->totalKilos +=  ($item["detExplotarUnidad"]*$item["colocaCantidad"]);
                            }
                            else
                            {
                                $this->putTextRight(number_format($item["colocaCantidad"],2) . " PZS", 14.5);
                            }
...
//                             }
  		...
  		foreach ($rsOcp as $item)
  		{
  		    ...
  		    if ($item["idOtroCargo"] == 1 && $item["monto"] > 0)
			  	$tieneFlete = true;
  		    if (($hoja+1)  == $this->_totalHojas)
  		    {
		...
		if ($this->__primerPedido && ($hoja+1)  == $this->_totalHojas && $tieneFlete && $this->__recogeEntrega == "ENTREGA")
		{
			$this->nextRow();
			$this->SetTextColor(255, 0, 0);
  		...
  		if (($hoja+1)  == $this->_totalHojas)
  		{
  		    $this->nextRow();
  		    $this->nextRow();
  		    $this->putTextRight("KG Totales", 35);
  		    
  		    $this->putTextRight(number_format($this->totalKilos,2), 14.5);
  		    $this->nextRow();
...
{
	$this->SetFont('Arial','',35);  
	$this->SetTextColor(133,167,208);
	$this->RotatedText(10,200, $this->__rangoCliente,0);
...
{	  
	...
	$this->SetTextColor(133,167,208);
	$this->RotatedText(10,200,'RECOGE SUCURSAL',0);
	$this->RotatedText(105,200, $this->__rangoCliente,0);
	$this->SetFont('Arial','',20);  
	$this->RotatedText(10,212,"FECHA COMPROMISO ".str_replace("00:00:00", "", $this->__fechaCompromiso),0);
...
if (intval($pedido->getPedidoDato("idCliente")) > 1)
{
	$pdf->__primerPedido = $pedido->esPrimerPedido(intval($pedido->getPedidoDato("idCliente")), $idPedido);
}
...
$pdf->__nombre =mb_convert_encoding(mb_strtoupper($pedido->getPedidoDato("cteNombre") . " " . $pedido->getPedidoDato("cteApellidos")), 'ISO-8859-1', 'UTF-8');
$pdf->__fecha = mb_convert_encoding(mb_strtoupper($pedido->getPedidoDato("fecha_capturado")), 'ISO-8859-1', 'UTF-8');
$pdf->__domicilio = mb_convert_encoding(mb_strtoupper($pedido->getPedidoDato("cteDomicilio1") . " " . $pedido->getPedidoDato("cteDomicilio2") . " " . $pedido->getPedidoDato("cteNumero") . " " . $pedido->getPedidoDato("cteColonia")), 'ISO-8859-1', 'UTF-8');
...
$pdf->__rangoCliente =mb_convert_encoding(mb_strtoupper( $pedido->getPedidoDato("rangoCliente")), 'ISO-8859-1', 'UTF-8');
