...
// foreach ($pesosCalibresPies as $datoca)
    ...
    $_lastUpdatedPrecios = $c->lastCheckUpdatePrecios();
    
    $_lastIdPedidoChecked = $c->lastIdPedidoChecked();
...
$query = "
    SELECT idPedido, colocado, idSucursalCapturado, idSucursalPreferenciaRecoge 
    FROM pedido 
    WHERE ( (estado = 'CAPTURADO' and idcliente <> 137) or (estado = 'AUTORIZADO' and idcliente = 137) ) 
    AND colocado = 'NO' 
    AND (idSucursalCapturado <> 0 OR idSucursalPreferenciaRecoge > 0) 
    AND idPedido > ".$_lastIdPedidoChecked."    

";

if ($c->haCambiadoPreciosEnProductosORollos())
{
    $query .= "
        UNION 
        SELECT idPedido, colocado, idSucursalCapturado, idSucursalPreferenciaRecoge 
        ...
        AND colocado = 'NO' 
        AND (idSucursalCapturado <> 0 OR idSucursalPreferenciaRecoge > 0) 
        AND pedidoHaCambiadoDePrecio(idPedido, , '".substr($c->lastCheckUpdatePrecios(), 0, 16)."') = 'SI'
    ";
}

$lstPedidosAProcesar = $pedido->getDataSet($query);
$c->updateLastCheckUpdatePrecios();

// $pedido->getAll("idPedido, colocado, idSucursalCapturado, idSucursalPreferenciaRecoge ",

//     "",

//     " ( (estado = 'CAPTURADO' and idcliente <> 137) or  (estado = 'AUTORIZADO' and idcliente = 137) )
//  AND colocado = 'NO' AND  (idSucursalCapturado <> 0 OR idSucursalPreferenciaRecoge > 0) ",
...

{
    ...
    $lastPedidoProcesado = $idPedidoProcesando;                 
...

    {
        ...
        // PedidoTrackingManager::logInfo($idPedidoProcesando, "Inicio de proceso para Autorizar Pedido en automático");
        PedidoTrackingManager::add($idPedidoProcesando, PedidoTrackingManager::$INFO, "Inicio de proceso para Autorizar Pedido en automático");
        ...
        myLog(" ___");
        myLog("Renglones Pedido: <h3>" . $cuantosRenglones . "</h3>");
        ...
        // PedidoTrackingManager::logWarning($idPedidoProcesando, "Validando si toda la mercancía del pedido puede surtirse para la sucursal: ". $idSucursal);
        PedidoTrackingManager::add($idPedidoProcesando, PedidoTrackingManager::$WARNING, "Validando si toda la mercancía del pedido puede surtirse para la sucursal: ". $idSucursal);
...

                    {
...

                        {
...

                            {
                                ...
                                // PedidoTrackingManager::logError($idPedidoProcesando, "No hay suficiente mercancía para surtir el Pedido: ". $viewProducto->getFullDescripcion());
                                PedidoTrackingManager::add($idPedidoProcesando, PedidoTrackingManager::$ERROR, "No hay suficiente mercancía para surtir el Pedido: ". $viewProducto->getFullDescripcion());
...

        {
            ...
            {
                // PedidoTrackingManager::logSuccess($idPedidoProcesando, "El Pedido ha sido Asignado de manera automática");
                PedidoTrackingManager::add($idPedidoProcesando, PedidoTrackingManager::$SUCCESS, "El Pedido ha sido Asignado de manera automática");

                $pedidoProcesando->setColocadoSI();

                $pedidoProcesando->setEstadoAUTORIZADO();
                $pedidoProcesando->setTipoAutorizacionAUTOMATICO();
                $pedidoProcesando->setObservacionAutoriza("Autorizado en Automático por proceso");

                $pedidoProcesando->setColocadoAutomaticoSI();

                

                // PedidoTrackingManager::logSuccess($idPedidoProcesando, "El Pedido se ha Autorizado de manera automática");
                PedidoTrackingManager::add($idPedidoProcesando, PedidoTrackingManager::$SUCCESS, "El Pedido se ha Autorizado de manera automática");
		...
		myLog(" - - - - - - - -    VAMOS A ver si se GENERAN V A L E S  D E   S A L I D A ");
		...
		myLog();
                if ($pedidoProcesando->getRecogeentrega() == "RECOGE")
                {
                    ...
                    // PedidoTrackingManager::logSuccess($idPedidoProcesando, "Se han generado Vales de Salida en Automático para el pedido");
                    PedidoTrackingManager::add($idPedidoProcesando, PedidoTrackingManager::$SUCCESS, "Se han generado Vales de Salida en Automático para el pedido");
                }
            ...
            {
                // PedidoTrackingManager::logError($idPedidoProcesando, "Ocurrió un error al intentar Asignar el pedido de manera automática");
                PedidoTrackingManager::add($idPedidoProcesando, PedidoTrackingManager::$ERROR, "Ocurrió un error al intentar Asignar el pedido de manera automática");
...

        {
            ...
            // PedidoTrackingManager::logError($idPedidoProcesando, "El Pedido no se pudo Autorizar de manera automática");    
            PedidoTrackingManager::add($idPedidoProcesando, PedidoTrackingManager::$ERROR, "El Pedido no se pudo Autorizar de manera automática");
...
$c->updateLastIdPedidoChecked($lastPedidoProcesado);
