<?php
$titlePage = "Reportes";
$breadCum = "Comisiones de Promotor";
...
<div class="modal inmodal" id="modalComisiones" tabindex="-1" role="dialog" aria-hidden="true" data-backdrop="static">
<!-- <div> -->
...
<!-- 				<i class="fa fa-shopping-cart modal-icon"></i> -->
				<h4 class="modal-title">Detalles Comisiones Pedido {{ comIdPedido }}</h4>
				Cliente: <strong>{{ comCliente }}</strong><br>
				Promotor: <strong>{{ comPromotor }}</strong><br>
				Vendedor: <strong>{{ comVendedor }}</strong><br>
				Subtotal: <strong>${{ formatNumber(comSubtotal) }} </strong>&nbsp;&nbsp;&nbsp;&nbsp;
				Otros Cargos: <strong>${{ formatNumber(comOtrosCargos) }} </strong>&nbsp;&nbsp;&nbsp;&nbsp;
				Descuento: <strong>${{ formatNumber(comDescuento) }} ({{comPorDescuento}}%)</strong> 
...
<!-- 				<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12"> -->
					<div class="table-responsive">
						<table class="table table-hover table-bordered ">
							...
							<tbody>
								<tr v-for="item in comDetalle">
									<td>{{ item.renglon }}</td>
									<td>{{ item.idProducto }}</td>
									<td>{{ item.detProducto }}</td>
									<td>{{ item.cantidad }}</td>
									<td class="text-right">{{ item.total }}</td>
									<td>{{ item.tipoprecio }}</td>
									<td class="text-right">{{ item.porcomision }}</td>
									<td class="text-right">{{ item.comision }}</td>
...
<div v-show="!seleccionandoPromotor">
	<button v-show="showBotonOtroPromotor" @click.prevent="seleccionarOtroPromotor" class="btn btn-warning">Seleccionar Otro Promotor</button>
	<br>
				...
				<div  class="row">										
					<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
						<div class="form-group">
							<label class="control-label">Tipo Comisi&oacute;n</label>
...
<!--                                             <th>Comisi&oacute;n Pagada</th> -->
									...
									<tbody >
											...
											<tr v-for='item in pedidos'>
												...
												<td :class="'text-right ' + (item.comisionpagada == 'NO' ? 'text-navy' : 'text-success')">$ {{ formatNumber(item.total) }}</td>
												<td :class="'text-right ' + (item.comisionpagada == 'NO' ? 'text-navy' : 'text-success')">$ {{ formatNumber(item.comisiones) }} 
<!-- 													<button @click.prevent="mostrarDetalleComisiones(item.idPedido)" class="btn btn-info btn-xs"><i class="fa fa-list-ol"></i></button>   -->
												</td>
												<td>{{ item.estado }}</td>
                                                ...
                                                <td>
                                                	<span v-show="item.miCliente == 'SI'" class="label label-primary">{{ item.idCorteComision }}</span>
													<span v-show="item.miCliente == 'NO'" class="label label-warning">{{ item.idCorteComisionVendedor }}</span>
