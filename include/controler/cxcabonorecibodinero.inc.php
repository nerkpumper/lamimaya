...
// 	$r->mostrarMsgs($debug);
	...
	function cargarClientes($page, $pageSize, $filtroNombreCliente, $chkConSaldo)
	{
		global $objSession;
		$r = new xajaxResponse();
		...
		$queryCount = "SELECT COUNT(*) cuantos
					FROM cliente 
					WHERE".
					($filtroNombreCliente !== "" ? " CONCAT(cliente.nombre, cliente.apellidos) LIKE '%".$filtroNombreCliente."%' AND " : "") . "
					getSaldoReciboDinero(cliente.idCliente)>0
					ORDER BY idCliente asc";
		
		$query = "SELECT cliente.idCliente, cliente.nombre, cliente.apellidos,  getTotalSaldosClientePedidosEntregados(idCliente)as saldoEntregados, getSaldoReciboDinero(cliente.idCliente)as saldoReciboDinero 
					FROM cliente 
					WHERE".
					($filtroNombreCliente !== "" ? " CONCAT(cliente.nombre, cliente.apellidos) LIKE '%".$filtroNombreCliente."%' AND " : "") . "
					getSaldoReciboDinero(cliente.idCliente)>0
			...
			$queryCount = "SELECT COUNT(*) cuantos
					FROM cliente 
					".
					($filtroNombreCliente !== "" ? "WHERE CONCAT(cliente.nombre, cliente.apellidos) LIKE '%".$filtroNombreCliente."%'  " : "") . "
		...
		foreach ($lst as $c)
		{
			$pushes .= "
						...
						app.clientes.push({
							...
							saldoEntregado: ".$c["saldoEntregados"].",
							saldoReciboDinero: ".$c["saldoReciboDinero"].",
	...
	function cargarPedidos($idCliente, $quePedidos, $page, $pageSize)
	{
		...
		$queryPage = "SELECT datos.*, getTotalCargosPedido(idPedido) as cargos, 
						...
						FROM (
							SELECT idCliente, idPedido, fecha_capturado, fecha_entregado,  factura, 
	...
	function verificarDisponiblePagoPedido($idPedido)
	{
		...
		$r->script("mdlExitWait(); $('#modalPagarConReciboDinero').modal();  ");
	...
	function pagarConReciboDinero($idPedido, $abonoMonto)
	{
		...
		$r->script(" mdlExitWait(); setTimeout(function() { app.refreshCliente(); }, 250); ");
