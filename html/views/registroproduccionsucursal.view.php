...
$_addHead="
 		<link href='".URL_BASE."assets/inspinia/css/plugins/bootstrapTour/bootstrap-tour.min.css' rel='stylesheet'>
 		    
 		";


$_addScript="
 		<script src=\"".URL_BASE."assets/inspinia/js/plugins/bootstrapTour/bootstrap-tour.min.js\"></script>
...
<style>
    ...
    #terminaelrollo .btn-end { display: block; }
</style>


<div id="wrapper">
<!-- 2A431467JGP04 -->
<!-- <button @click.prevent="empiezatour">tour</button> -->
...
<!-- 						<td>{{ remi.fecha }}</td> -->
						<td>{{ remi.remision }}</td>
						<td class="h4 text-navy">{{ remi.almacen }}</td>
						<td>{{ formatNumber(remi.kilos) }}</td>
						<td>{{ formatNumber(remi.disponible) }}</td>
						<td >
							<button  v-show="remi.puedeSeleccionar  == 'SI'" class="btn btn-primary btn-xs" @click.prevent="seleccionarNoRemision(indexrollo, indexnorollo)">Seleccionar</button>
							<span v-show="remi.puedeSeleccionar  == 'NO'" class="badge badge-danger">Este Rollo no pertenece a su Sucursal</span>
						</td>
					</tr>
				</tbody>
			</table>
...
<!-- 			<span class="label label-warning pull-right">Data has changed</span> -->
		...
		<div class="ibox-content">
			<div class="row">
				...
				<div v-show="showGuardarEspesor" class="col-lg-4 col-md-4 col-sm-12 col-xs-12 m-t m-b">
			
				
											<p>Espesor Rollo (mm)</p>
											<input type="text" class="form-control " v-model="espesor"
												v-on:keypress.enter="app.cargarPedidoDetalle();"
												oninput="this.value = this.value.replace(/[^0-9\t]/d, '').replace(/(\..*)\./d, '$1');">
												<p class="text-danger">Para poder terminar rollo capture espesor</p>
											<button @click.prevent="guardarEspesor" class="btn btn-warning btn-sm ">Guardar espesor</button>
				</div>
			
				<div class="ibox-content">
                            <h4>
                                Agregar largo de Rollo
                            </h4>
                            <div class="switch">
                                <div class="onoffswitch">
                                    <input type="checkbox" checked class="onoffswitch-checkbox" id="example1" v-model =showGuardarlargoRollo>
				...
				<div v-show="showGuardarlargoRollo" class="col-lg-4 col-md-4 col-sm-12 col-xs-12 m-t m-b">	


											<p>Ingrese el largo de rollo</p>
											<input type="text" class="form-control " v-model="largoRollo"
												v-on:keypress.enter="app.cargarPedidoDetalle();"
												oninput="this.value = this.value.replace(/[^0-9\t]/d, '').replace(/(\..*)\./d, '$1');">
											<button @click.prevent="guardarLargoRollo" class="btn btn-warning btn-sm ">Guardar largo de rollo</button>
							
				
				</div>	
				<button  v-show="!showGuardarEspesor" class="btn btn-success" @click.prevent="concluirRegistroProduccion"><i class="fa fa-check"></i> Concluir Registro de Producci&oacute;n</button>
				</div>
				</div>
