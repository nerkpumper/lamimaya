...
// 				$btnFactura = "&nbsp;<button class='btn btn-primary btn-xs' onclick='fnSolicitarFactura(".$row["idPedido"].");'><i class='fa fa-gavel'></i> Soliciturar Factura</button>";
				$btnFactura = "&nbsp;<a class='btn btn-primary btn-xs' href='".URL_BASE."/pedidofacturar/".$row["idPedido"]."'><i class='fa fa-gavel'></i> Soliciturar Factura</a>";
			}
			else 
			{
				if ($row["facturado"] == "SI")
...
// 	$r->mostrarMsgs($debug);
	...
	function autorizarPedido($idPedido, $observacion)
	{
		...
		{
			$r->script("saError(Ha ocurrido un error. " . utf8_encode($pedido->getStrError()).");");
		}
...
// 		$pedido->setDateAndUser("autorizado");
		...
		if (!$pedido->getError())
		{
			...
			$r->saSuccess("Se ha cambiado la Observación de Autorización de forma correcta.");
...
// 			//agregar validación para despachar solo LISTOS PARA PRODUCIR
