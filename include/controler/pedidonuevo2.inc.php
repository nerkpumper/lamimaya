...
// 	$r->mostrarAviso($debug);
	...
	function loadCotizacion($idCotizacion)
	{
		...
		$datosCotizacion = "setTimeout (function () {
			...
			app.pedidoSoloParaObra = ".($cotizacion->getRecogeentrega() == "OBRA" ? "true" : "false").";
...
// 		if ($objSession->getIdUsuario() == 10 || $objSession->getIdUsuario() == 15)
	...
	function cargarClienteMostador($idCliente)
	{
		...
		$r->script("  setTimeout(function(){  mdlExitWait();  }, 1500); ");				
...
// 		$lst = $tipos->getAll("","","");
	...
	function cargarListaProductos($esParaObra = false)
	{
        global $objSession;
		$r = new xajaxResponse();
        // $r->startDebug();
		...
		$productos->getAllView($objSession->getIdUsuario(), $esParaObra);
...
// 		$r->mostrarAviso("cargarlistaproductos: 485"); return $r;
		...
		for($i = 0 ; $i <count($productos->lstProductos) ; $i++)
		{
			...
			if ($productos->lstProductos[$i]->getEstado() == "ACTIVO")
			{
				...
				//obra = AND idRollo > 1 and idUnidad = 1
				$soloObra = 'NO';
				if ($productos->lstProductos[$i]->getIdRollo() > 1 && 
				    $productos->lstProductos[$i]->getIdUnidad() == 1)
					{
						$soloObra = 'SI';
					}
...
// 		$debug = ob_get_clean();
        ...
        $r->script("  setTimeout(function(){ app.agruparProductosComercializados(); }, 800);");

        $r->script("  setTimeout(function(){ app.cargarProductosMasVendidos(); }, 850);");
        $r->script("  setTimeout(function(){ app.cargarProductosFavoritos(); }, 900);");
		$r->script("  setTimeout(function(){ mdlExitWait(); }, 950);");
...
// 					";
		...
		$r->script($strListadoProductos);
		// $r->mostrarAviso("Producto encontrado: " . $strListadoProductos );
