...
// 	$r->mostrarMsgs($debug);
	...
	function cargarConfiguracion()
	{
		...
		{
			$r->saError("No se han podido cargar los datos de configuraci�n de precios.");
		...
		$r->script("
					...
					app.configRangoAcryOpa1Inicio = '" . $config->getRangoMetros1AcryOpaInicio() . "';
					app.configRangoAcryOpa1Fin = '" . $config->getRangoMetros1AcryOpaFin() . "';
					app.configRangoAcryOpa2Inicio = '" . $config->getRangoMetros2AcryOpaInicio() . "';
					app.configRangoAcryOpa2Fin = '" . $config->getRangoMetros2AcryOpaFin() . "';
					app.configRangoAcryOpa3Inicio = '" . $config->getRangoMetros3AcryOpaInicio() . "';
					app.configRangoAcryOpa3Fin = '" . $config->getRangoMetros3AcryOpaFin() . "';

					app.configRangoMultipanel1Inicio = '" . $config->getRangoMetros1MultipanelInicio() . "';
					app.configRangoMultipanel1Fin = '" . $config->getRangoMetros1MultipanelFin() . "';
					app.configRangoMultipanel2Inicio = '" . $config->getRangoMetros2MultipanelInicio() . "';
					app.configRangoMultipanel2Fin = '" . $config->getRangoMetros2MultipanelFin() . "';
					app.configRangoMultipanel3Inicio = '" . $config->getRangoMetros3MultipanelInicio() . "';
					app.configRangoMultipanel3Fin = '" . $config->getRangoMetros3MultipanelFin() . "';
				    ...
				    app.configComision1Rango4 = '". $config->getComision1R4() ."';
				    ...
				    app.configComision2Rango4 = '". $config->getComision2R4() ."';
				    ...
				    app.configComision3Rango4 = '". $config->getComision3R4() ."';
	...
	function guardarConfiguracion($rm1Inicio, $rm1Fin, $rm2Inicio, $rm2Fin, $rm3Inicio, $rm3Fin)
	{
		...
		if ($config->getIdConfiguracion() <= 0)
		{
			$r->saError("No se han podido actualizar los datos de configuración de rangos.");
			//$r->redirect(URL_BASE . "lamina", 2);
			return $r;
		}
...
// 			$r->script("setTimeout(function(){ ocultarMensaje();}, 2000);");
	...
	function guardarConfiguracionAcryOpa($rm1Inicio, $rm1Fin, $rm2Inicio, $rm2Fin, $rm3Inicio, $rm3Fin)
	{
		$r = new xajaxResponse();
		...
		$config->setRangoMetros1AcryOpaInicio($rm1Inicio);
		$config->setRangoMetros1AcryOpaFin($rm1Fin);

		$config->setRangoMetros2AcryOpaInicio($rm2Inicio);
		$config->setRangoMetros2AcryOpaFin($rm2Fin);

		$config->setRangoMetros3AcryOpaInicio($rm3Inicio);
		$config->setRangoMetros3AcryOpaFin($rm3Fin);
		$config->setDateAndUser("modificacion");
		...
		else
		{
			$r->script("
					...
					app.editRangoAcryOpa = false;
					swal(\"Rangos Acrylit Opalit\",
					     \"Los cambios han sido realizados correctamente.\",
					     \"success\");
...
// 			$r->script("setTimeout(function(){ ocultarMensaje();}, 2000);");
	...
	$xajax->registerFunction("guardarConfiguracionAcryOpa");

	function guardarConfiguracionMultipanel($rm1Inicio, $rm1Fin, $rm2Inicio, $rm2Fin, $rm3Inicio, $rm3Fin)
	{
		$r = new xajaxResponse();
		...
		$config->setRangoMetros1MultipanelInicio($rm1Inicio);
		$config->setRangoMetros1MultipanelFin($rm1Fin);

		$config->setRangoMetros2MultipanelInicio($rm2Inicio);
		$config->setRangoMetros2MultipanelFin($rm2Fin);

		$config->setRangoMetros3MultipanelInicio($rm3Inicio);
		$config->setRangoMetros3MultipanelFin($rm3Fin);
		$config->setDateAndUser("modificacion");
		...
		else
		{
			$r->script("
					...
					app.editRangoMultipanel = false;
					swal(\"Rangos Multipanel\",
					     \"Los cambios han sido realizados correctamente.\",
					     \"success\");
...
// 			$r->script("setTimeout(function(){ ocultarMensaje();}, 2000);");
	...
	$xajax->registerFunction("guardarConfiguracionMultipanel");
	...
	function guardarConfiguracionPrecioXCalibre($precioCal20, $precioCal22, $precioCal24, $precioCal26, $precioCal28)
	{
		...
		if ($config->getIdConfiguracion() <= 0)
		{
			$r->saError("No se han podido actualizar los datos de configuraci�n de precios.");
			//$r->redirect(URL_BASE . "lamina", 2);
			return $r;
		}
	...
	function guardarConfiguracionPedido($pedidoDescuento)
	{
		...
		if ($config->getIdConfiguracion() <= 0)
		{
			$r->saError("No se han podido actualizar los datos de configuraci�n de pedido.");
			//$r->redirect(URL_BASE . "lamina", 2);
			return $r;
		}
	...
	function guardarConfiguracionComisiones($config1Rango1, $config1Rango2, $config1Rango3, $config1Rango4,
											$config2Rango1, $config2Rango2, $config2Rango3, $config2Rango4,
											$config3Rango1, $config3Rango2, $config3Rango3, $config3Rango4)
	{
		$r = new xajaxResponse();
		...
		if ($config->getIdConfiguracion() <= 0)
		{
			$r->saError("No se han podido actualizar los datos de configuraci�n de comisiones.");
			//$r->redirect(URL_BASE . "lamina", 2);
			return $r;
		}
