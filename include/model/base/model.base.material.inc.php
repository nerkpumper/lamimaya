<?php
	...
	class ModeloBaseMaterial extends clsBasicCommon
	{
		#------------------------------------------------------------------------------------------------------#
		#----------------------------------------------Propiedades---------------------------------------------#
		#------------------------------------------------------------------------------------------------------#
		var $_nombreClase="base.ModeloBaseMaterial";
		...
		{
			if($idMaterial==0||$idMaterial==""||!is_numeric($idMaterial)|| (is_string($idMaterial)&&!ctype_digit($idMaterial)))return $this->setError("Tipo de dato incorrecto para idMaterial.");
		...
		protected function Insertar()
		{
			try
			{
				...
				if(!$result)
					return $this->setSystemError("Error en la insercion de registro.","[" . $SQL . "][" . mysqli_error($this->dbLink) . "][ModeloBaseMaterial::Insertar]");
				
				$this->idMaterial=mysqli_insert_id($this->dbLink);
				return true;
			}
			catch (Exception $e)
			{
		...
		protected function Actualizar()
		{
			try
			{
				...
				$result=mysqli_query($this->dbLink,$SQL);
				if(!$result)
					return $this->setSystemError("Error en la actualizacion de registro.","[" . $SQL . "][" . mysqli_error($this->dbLink) . "][ModeloBaseMaterial::Update]");
		...
		public function Borrar()
		{
			...
			try
			{
				...
				WHERE idMaterial=" . mysqli_real_escape_string($this->dbLink,$this->idMaterial);
				$result=mysqli_query($this->dbLink,$SQL);
				if(!$result)
					return $this->setSystemError("Error en el borrado de registro.","[" . $SQL . "][" . mysqli_error($this->dbLink) . "][ModeloBaseMaterial::Borrar]");
			}
			catch (Exception $e)
			{
				return $this->setErrorCatch($e);
		...
		public function getDatos()
		{
			try
			{
				...
				$result=mysqli_query($this->dbLink,$SQL);
				if(!$result)
					return $this->setSystemError("Error en la obtencion de detalles de registro.","[ModeloBaseMaterial::getDatos][" . $SQL . "][" . mysqli_error($this->dbLink) . "]");
