...
// 			app.pedidoDetalle[app.indexDespachando].despachado = 'SI';
	...
	function darSalidaAInventarioFromStockNoRollo($idPedidoDetalle, $idProducto, $idPedido, $cantidad, $idRemisionRollo, $proIdTipoProducto, $proIdUnidad, $proIdRollo, $detCantidadReal)
	{
		$r = new xajaxResponse();
// $r->starDebug();
		$producto = new ModeloProducto();
...
// 			echo "<br>antes set idpedidodetalle";
			$pd->setIdPedidoDetalle($idPedidoDetalle);
			...
			{
// 			    echo "<br>gettotalexplotar - getexplotadoreal";
				if (($pd->getTotalExplotar() - $pd->getExplotadoReal()) <= 0)
				{
					$pd->setDespachadoSI();
					$pd->setDateAndUser("despachado");
		...
		{
// 		    echo "<br> Error al insertar inv";
		    $errores .= $inv->getStrError();
			$blnDoCommit = false;
		}
...
// 	    }
	...
	function darSalidaAInventarioFromStockNoRolloLaminaYScrap($idPedidoDetalle, $idProducto, $idPedido, $cantidad, $idRemisionRollo, $noMolduras)
	{
	    ...
	    if (!$inv->getError())
	    {
	        ...
	        $invMol->setObservaciones(mb_convert_encoding("Surtido de Pedido Molduras (de L�mina, 'UTF-8', 'ISO-8859-1')"));
