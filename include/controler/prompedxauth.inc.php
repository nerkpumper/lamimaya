...
// 	$r->mostrarMsgs($debug);
	...
	function cargarPedidoCreditos($idPedido){
		...
		$datos = $pedido->getAll("pedido.idPedido, pedido.total, pedido.saldo, pedido.estado, pedido.tipocliente,
		...
		$r->script("
                ...
                app.frmXCredito.saldoPedido = ".$datos["saldo"].";
                app.frmXCredito.pedidoGrande = ".($datos["total"] >= 150000 ? "true" : "false").";
                ...
                app.frmXCredito.cteTipoCliente = '".$datos["tipocliente"]."';                   
	
				app.frmXCredito.promoCredito = ".$datos["promoCredito"].";
				app.frmXCredito.promoCredito50 = ". ($datos["total"] >= 150000 ? $datos["promoCredito"]/4 : $datos["promoCredito"]/2) .";
				app.frmXCredito.promoCredito25 = ".($datos["promoCredito"]/4).";
...
// 		$objSession->getIdUsuario()
		...
		if ($filtro["noPedido"] == "")
		{
			...
			$lst = $pedido->getAll("pedido.idPedido, concat(cliente.nombre, ' ', cliente.apellidos) as nombrecliente, pedido.total, pedido.fecha_capturado, cliente.saldarpedidoparaautorizar, pedido.saldada ",
					               "inner join cliente 
			...
			foreach ($lst as $item)
			{
				$pushes .= "
						...
						app.pedidos.push({
                            ...
                            saldarpedidoparaautorizar: '".$item["saldarpedidoparaautorizar"]."'
						});
		...
		{		
			...
			$lst = $pedido->getAll("pedido.idPedido, cliente.idUsuarioPromotor, pedido.estado, concat(cliente.nombre, ' ', cliente.apellidos) as nombrecliente, pedido.total, pedido.saldo, pedido.fecha_capturado,  cliente.saldarpedidoparaautorizar, pedido.saldada  ",
					"inner join cliente
									  on cliente.idCliente = pedido.idCliente",
			...
			foreach ($lst as $item)
			{
				...
				if ($item["idUsuarioPromotor"] == $objSession->getIdUsuario() || ModeloUsuario::amIRoot())
				{
					if ($item["estado"] == "CAPTURADO")
					{
						$pushes .= "
