...
// 	$r->mostrarMsgs($debug);
    ...
    function cargarPromotores()
    {
        ...
        if(Permisos::userIsThisRol(Permisos::$rol_PROMOTOR)  || Permisos::userIsThisRol(Permisos::$rol_PROMOTORPRODUCCION) || Permisos::userIsThisRol(Permisos::$rol_CXCVENTAS)  || Permisos::userIsThisRol(Permisos::$rol_PROMOTORPLUSPRODUCCION))
        {
...
// $r->mostrarAviso($image);
        ...
        if ($indexPromotor >= 0)
        {
            $r->script(" app.preparePromotor(".$indexPromotor."); app.showBotonOtroPromotor = false; ");
        }
...
// 			$r->saInfo("No se encontró información.");
	...
	function generarCorteComision($promotor, $pedidos, $deducciones, $totalAPagar, $totalDeducciones, $netoAPagar)
	{
		$r = new xajaxResponse();
		$doCommit = true;
		$errores = "";
		...
		$cc->setAPagar($cc->getTotal() - $cc->getComisionAdelantada());
		...
		if ($doCommit)
		{
		    ...
		    $r->saSuccess("Se ha generado el Corte de Comisi�n: " . $cc->getIdCorteComision());
...
// 	        $addWhere .= "  AND date_format(cxccuentacomision.fecha_creacion, '%Y-%m-%d') BETWEEN '".$desde."' AND '".$hasta."' ";
