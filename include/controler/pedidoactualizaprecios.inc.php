...
// 	$r->mostrarAviso($debug);
	...
	function loadPedido($idPedido)
	{
		...
		$sql = "SELECT pedidoBloqueadoXPrecios(".$idPedido.") bloqueado";

		$bloqueado = $pedido->getDataset($sql)[0]["bloqueado"];

		if ($bloqueado == 'NO')
		{
			$r->saInfo("El Pedido que esta intentando cargar no esta Bloqueado por Precios.");
			$r->script("app.vistaPedido = false; app.pedidoDesbloqueado = true; mdlExitWait(); setTimeout( function(){ window.close(); }, 2000 );");
			// $r->script(" mdlExitWait();  app.vistaPedido = false; ");
			return $r;
		}
		...
		// if (!Permisos::userIsThisRol(Permisos::$rol_ADMINISTRADOR) && $idUsuario != $cliente->getIdUsuarioPromotor() && $idUsuario != $pedido->getId_usuario_capturado())
		if (Permisos::userIsThisRol(Permisos::$rol_ADMINISTRADOR) && Permisos::userIsThisRol(Permisos::$rol_ROOT)  && !Permisos::userIsThisRol(Permisos::$rol_ADMINISTRADOR) && $idUsuario != $cliente->getIdUsuarioPromotor() )
		{
			$r->saError("El Pedido que esta intentando cargar no pertenece a sus clientes o NO ha sido capturado por usted.");
			$r->script(" mdlExitWait();  app.vistaPedido = false; ");
			return $r;
		}

		if (Permisos::userIsThisRol(Permisos::$rol_ADMINISTRADOR) || Permisos::userIsThisRol(Permisos::$rol_ROOT))
		{
			$r->script(" app.isAdministrador = true; ");
		}
		
		//Cargamos cliente
		$r->script(" app.idClienteSeleccionado = ".$pedido->getIdCliente()."; 
					 setTimeout(function() { app.refreshDatosClienteSeleccionado('".$cliente->getRangoCliente()."');}, 50);   ");
		...
		$rangosString = $pedido->getRangosString();
		if ($rangosString == "")
		{
			$rangosString = "1111111111";
		}

					 
		$datosPedido = "setTimeout (function () {
			app.subtotalPedido = ".$pedido->getSubtotal()."; 
			app.subtotalPedidoOriginal = ".$pedido->getSubtotal()."; 
			app.totalOtrosCargosOriginal = ".$pedido->getOtrosCargos() ."; 
			app.ivaPedido = ".$pedido->getIva()."; 
			app.descuentoPedido = ".$pedido->getDescuento()."; 
			app.totalPedido = ".$pedido->getTotal()."; 
			app.totalPedidoOriginal = ".$pedido->getTotal()."; 
			app.observacionPedido = '".$pedido->getObservacionCaptura()."';
			app.tipoPrecioGalvamex = 1; 
			app.selRecogeRecibe = '".$pedido->getRecogeentrega()."'; 
			app.selSucursalPreferencia = ".$pedido->getIdSucursalPreferenciaRecoge()."; 
			app.ctePersona = '".$pedido->getPersonaEntrega()."';
			app.cteDireccion = '".$pedido->getDomicilioEntrega()."';
			app.cteNumero = '".$pedido->getNumeroEntrega()."';
			app.cteColonia = '".$pedido->getColoniaEntrega()."';
			app.cteCiudad = '".$pedido->getCiudadEntrega()."';
			app.horaEntrega = '".$pedido->getHoraRecibe()."'; 
			app.fechaAbierta = '".$pedido->getFechaAbierta()."'; 
			app.pedidoExpress = '".$pedido->getPedidoExpress()."'; 
			

			app.clienteTipoRangoSeleccionado = '".$cliente->getRangoCliente()."';

			$(\"#dtFechaEntrega\").val('".clsUtilerias::formatoFecha(substr($pedido->getFechaCompromiso(), 0, 10))."');
	
			}, 1000);";


		$r->script($datosPedido);

		$oc = new ModeloOtrocargo();
	    	    
	    $lst = $oc->getAll("otrocargo.idOtroCargo, otrocargo.descripcion, otrocargo.ingreso, otrocargo.solicitar, otrocargo.precioIngreso,       
							IFNULL(ocp.cantidadIngreso, '') cantidadIngreso, IFNULL(ocp.monto, 0) monto ", 
							"LEFT JOIN otroscargospedido ocp
							ON ocp.idotrocargo = otrocargo.idOtroCargo
							AND ocp.idPedido = " .$idPedido, "" ," idOtroCargo");
		...
		for($i = 0 ; $i <count($pedido->lstProductos) ; $i++)
		{
			$producto = new ModeloViewproductos();
			$producto = $pedido->lstProductos[$i];
			...
			if ($producto->getEstado() == "ACTIVO")
			{
				...
				if ($idProducto != $molIdProducto && $idProducto != $molIdMaquila)
				{
					...
					// $strListadoProductos .= "		precio1:  '" .$producto->getPrecio1() . "', ";
					// $strListadoProductos .= "		precio2:  '" .$producto->getPrecio2() . "', ";
					// $strListadoProductos .= "		precio3:  '" .$producto->getPrecio3() . "', ";
					// $strListadoProductos .= "       preciomendez:  '" .$producto->getPreciomendez() . "', ";

					$strListadoProductos .= "		precio1:  '" .$renglon->getPrecioUnitario() . "', ";
					$strListadoProductos .= "		precio2:  '" .$renglon->getPrecioUnitario() . "', ";
					$strListadoProductos .= "		precio3:  '" .$renglon->getPrecioUnitario() . "', ";
					$strListadoProductos .= "       preciomendez:  '" .$renglon->getPrecioUnitario() . "', ";
					...
					$strListadoProductos .= "        precioRenglonOriginal: '".$renglon->getPrecioUnitario()."', ";
					$strListadoProductos .= "		rangoRenglon: '1', ";
					$strListadoProductos .= "        totalRenglon: '0', ";
					$strListadoProductos .= "        totalRenglonOriginal: '".$renglon->getTotal()."', ";
					...
					$strListadoProductos .= "		idPedidoDetalle:  " . $renglon->getIdPedidoDetalle();
		...
		// $r->mostrarAviso($datosCotizacion);
		...
		$r->script("setTimeout(function(){ app.calculaTotales(); }, 500); 
					...
					setTimeout( function(){ app.refreshDatosClienteSeleccionado('".$cliente->getRangoCliente()."'); mdlStatusWait('Recuperando Cliente'); 
					...
					setTimeout(function(){
						...
						app.selDescuentoIndividual = '".$pedido->getDescuentoIndividual()."'; 
						app.calculaTotales();
                    }, 1900);
					...
					setTimeout( function() { app.updateAllToCurrentPrices(false);  },3200);
					

					//setTimeout( function() {mdlExitWait();  },3500);
					
					");

					

// $r->mostrarAviso("todo bien line: " . __LINE__); return $r;					
		// $r->endDegug();
	    return $r;
	}
	$xajax->registerFunction("loadPedido");
	...
	function loadCotizacion($idCotizacion)
	{
		...
		$r->script(" app.idClienteSeleccionado = ".$cotizacion->getIdCliente()."; 
					 setTimeout(function() { app.refreshDatosClienteSeleccionado('".$cotizacion->getRangoCliente()."');}, 500);   ");
		...
		$r->script("setTimeout(function(){ app.calculaTotales(); }, 500); 
					...
					setTimeout( function() {mdlExitWait(); $('.right-sidebar-toggle').click(); },2200);");
		// $r->endDegug();
	    return $r;
	}
...
// 		$r->mostrarAviso("cargarlistaproductos: 485"); return $r;
...
// 	                          ,$strPersona, $strDireccion, $strNumero, $strColonia, $strCiudad, $fechaEntrega, $horaEntrega, $tipoPedido, $porDescuento, $maxDescuentoIndividual, $descuentoIndividual, $observacionPedido, $totalOtrosCargos, $otrosCargos
	...
	function levantarPedido($idClienteSeleccionado, $subtotalPedido, $ivaPedido, $descuentoPedido, $totalPedido, $tipoPrecioGalvamex ,$detalle, $recogeRecibe, $sucursalPreferencia, $strPersona, $strDireccion, $strNumero, $strColonia, $strCiudad, $fechaEntrega, $horaEntrega, $fechaAbierta, $pedidoExpress, $tipoPedido, $porDescuento, $maxDescuentoIndividual, $descuentoIndividual, $observacionPedido, $totalOtrosCargos, $otrosCargos, $molCostoDobles, $molCostoCorte, $buenfintipopago, $fechaEntregaPorDefinir, $tipoObra, $isCotizacion = false, $idCotizacion = 0,$getRangosString = "", $pasarAPedidoLaCotizacion = false, $previoAPasarPedidoACotizacion = false, $utilizarReciboDinero = false, $RDAmparaCotizacion = false, $RDATomar = 0)
	{
		global $objSession;
		$r = new xajaxResponse();
	...
	// }
			...
			// $r->mostrarAviso("Rangosstring " . $getRangosString ); return $r;
...
// 	    $r->endDegug();
	...
	function mostrarInventarioStock($index, $idProducto)
	{
	    ...
	    $lst = $invsuc->getAll(" sucursal.nombre sucursal, inventariosucursal.existencia, inventariosucursal.apartado, (inventariosucursal.existencia - inventariosucursal.apartado) disponible  ",
	        ...
	        " inventariosucursal.idproducto = ". $idProducto ."
                 ");
	    
// 	    and inventariosucursal.existencia > 0
	    
// 	    $r->mostrarAviso( $invsuc->getAllQUERY(" sucursal.nombre sucursal, inventariosucursal.existencia, inventariosucursal.apartado, (inventariosucursal.existencia - inventariosucursal.apartado) disponible  ",
// 	        "inner join sucursal on inventariosucursal.idsucursal = sucursal.idsucursal",
// 	        " inventariosucursal.idproducto = ". $idProducto ."
//                 and inventariosucursal.existencia > 0 ")); return $r;
	...
	function actualizarPedido($idClienteSeleccionado, $subtotalPedido, $ivaPedido, $descuentoPedido, $totalPedido, $tipoPrecioGalvamex ,$detalle, $recogeRecibe, $sucursalPreferencia, $strPersona, $strDireccion, $strNumero, $strColonia, $strCiudad, $fechaEntrega, $horaEntrega, $fechaAbierta, $pedidoExpress, $tipoPedido, $porDescuento, $maxDescuentoIndividual, $descuentoIndividual, $observacionPedido, $totalOtrosCargos, $otrosCargos, $molCostoDobles, $molCostoCorte, $buenfintipopago, $fechaEntregaPorDefinir, $tipoObra, $isCotizacion = false, $idCotizacion = 0,$getRangosString = "", $pasarAPedidoLaCotizacion = false, $previoAPasarPedidoACotizacion = false, $utilizarReciboDinero = false, $RDAmparaCotizacion = false, $RDATomar = 0, $idPedido = 0)
	{
		global $objSession;
		$r = new xajaxResponse();
		...
		// else
		// {
		// 	$r->mostrarAviso("Is Cotizacion false " . $isCotizacion . " ID PEDIDO: " . $idPedido); return $r;
...
// 		$r->mostrarAviso("error line 933"); return $r;
		...
		if (!$isCotizacion && $idPedido > 0)
		{
			$pedido->setIdPedido($idPedido);			
			// $r->mostrarAviso("Pedido Cargado " . __LINE__); return $r;
		}
...
// 		echo "ren 882";
		...
		if (!$isCotizacion)
		{
            ...
            // $pedido->setDateAndUser("capturado");
			$pedido->setFecha_updateprecios(date("Y-m-d H:i:s"));
			...
			else
			{
			    ...
			    if (!$isCotizacion)
			    {
    				...
    				foreach ($detalle as $item)
    				{
						...
						// $r->mostrarAviso("Cargaremos idPedidoDetalle: " . $item["idPedidoDetalle"]);

						$det->setIdPedidoDetalle($item["idPedidoDetalle"]);
    					$det->setIdPedido($id_pedido);
    					$det->setRenglon($renglon);
    					$det->setIdProducto($item["idProducto"]);
    // 					$det->setTipoPrecio(getTipoPrecioForDetail($item["tipoPrecio"], $tipoPrecioGalvamex, $item["isRango"]));
    ...
    // 					$det->setComision($item["rangoRenglon"]);
    					...
    					if ($det->getError())
    					{
    						...
    						break;
    							...
    							// $r->mostrarAviso("salir for " .$blnDoCommit); return $r;
			    }
			    else
			    {  // C O T I Z A C I O N
			        $id_pedido = $cotizacion->getIdCotizacion();
...
// 				Fin Otros Cargos
			...
			if ($blnDoCommit)
			{
			    ...
			    if (!$isCotizacion) 
				{
				// $r->mostrarAviso("commit ". __LINE__);
					// $r->script("mdlExitWait();");
					$r->saSuccess("El Pedido ha sido desbloqueado");
					$r->script("app.vistaPedido = false; app.pedidoDesbloqueado = true; mdlExitWait();");
...
// 				$r->script("mdlExitWait(); app.mostrarBotonGuardar = true;");
...
// $r->endDegug();
	...
	$xajax->registerFunction("actualizarPedido");
	...
	function desbloquearPedido($idPedido)
	{
		...
		// $r->redirect("index", 2);
		$r->script("app.vistaPedido = false; app.pedidoDesbloqueado = true; ");
