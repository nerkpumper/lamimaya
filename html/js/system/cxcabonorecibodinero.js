...
var app = new Vue({
	...
	methods: {
		deseoSaldarTodo: function(){
			...
			cargarLstPromotores: function(){
				        var vm = this;		
				        $.ajax({								
				           headers: getAxiosHeaders(),   				
				            url: URL_BASE + 'api/usuario.api.php?method=getUsuarioCapturaPedido',				
				                success:function(response){				
				                    vm.lstPromotores = response.lst;								
				                },				
		...
		loadCliPage: function(){
			xajax_cargarClientes(this.cliPage, this.cliPageSize, this.filtroNombreCliente, this.chkConSaldo);
		},
		...
		sendToExcel: function(){
			...
			sendToExcel("tablaToExcel", "Reporte", "Reporte Pedidos Cliente", "", 1);
			//alert("enviamos");
		},
