...
//		            	mostrarAviso(resp.msg);
		...
		preLevantarPedido: function(isCotizacion, pasarAPedidoLaCotizacion){
			...
			swal({
				...
				// alert("vamos");
				// mdlShowWait();
				setTimeout(app.levantarPedido(app.appisCotizacion, app.apppasarAPedidoLaCotizacion), 100);

		    });
		},
		levantarPedido: function(isCotizacion, pasarAPedidoLaCotizacion){
...
//						console.log("Producto Modificado: index-> " + (vindexModal));
			...
			// setTimeout(function() { app.checarSiHayMercanciaIsocindu(); }, 100);
		},
		...
		checarSiHayMercanciaIsocindu: function(){
			...
			for (i = 0; i < this.listadoPedido.length; i++)
			{
				if (this.listadoPedido[i].idAplicacion == 31)
				{
					...
					// if (str.includes("POLI"))
					{	
						setTimeout(function(){ saInfo("Hay Panel Isocindu en el listado de productos. Recuerda cotizar el flete."); }, 500);
...
//			}
			...
			if (this.molCantUnidad > 6.4 )
			{
				seguir = false;
				this.molError = this.molError + "<br>" + "Metros Lineales debe ser menor o igual a 6.4.";
			}
...
// }, 400);
			...
			setTimeout (function() { mdlExitWait(); }, 500 + 400);
		},
		updateAllToCurrentPrices: function(){
			...
			setTimeout (function() { mdlExitWait(); }, 500 + (400 * this.listadoPedido.length));
		}
