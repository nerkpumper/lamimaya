...
// 	$r->mostrarAviso($debug);
	...
	function loadCotizacion($idCotizacion)
	{
		...
		$r->script(" app.idClienteSeleccionado = ".$cotizacion->getIdCliente()."; 
					 setTimeout(function() { app.refreshDatosClienteSeleccionado();}, 50);   ");
		...
		$r->script("setTimeout(function(){ app.calculaTotales(); }, 500); 
					...
					setTimeout( function(){ app.refreshDatosClienteSeleccionado(); mdlStatusWait('Recuperando Cliente'); 
	...
	function cargarClienteById($idCliente)
	{
		...
		foreach ($lst as $row)
		{
			...
			$strClientes = "app.idClienteSeleccionado = ".$row["idCliente"].";
                            ...
                            app.clienteTipoRangoSeleccionado = '".$row["rangoCliente"]."';
...
// 	                          ,$strPersona, $strDireccion, $strNumero, $strColonia, $strCiudad, $fechaEntrega, $horaEntrega, $tipoPedido, $porDescuento, $maxDescuentoIndividual, $descuentoIndividual, $observacionPedido, $totalOtrosCargos, $otrosCargos
	...
	function levantarPedido($idClienteSeleccionado, $subtotalPedido, $ivaPedido, $descuentoPedido, $totalPedido, $tipoPrecioGalvamex ,$detalle, $recogeRecibe, $sucursalPreferencia, $strPersona, $strDireccion, $strNumero, $strColonia, $strCiudad, $fechaEntrega, $horaEntrega, $fechaAbierta, $pedidoExpress, $tipoPedido, $porDescuento, $maxDescuentoIndividual, $descuentoIndividual, $observacionPedido, $totalOtrosCargos, $otrosCargos, $molCostoDobles, $molCostoCorte, $buenfintipopago, $fechaEntregaPorDefinir, $tipoObra, $isCotizacion = false, $idCotizacion = 0,$getRangosString = "", $pasarAPedidoLaCotizacion = false)
	{
		global $objSession;
		$r = new xajaxResponse();
				...
				if ($blnDoCommit)
				{
					...
					if ($pasarAPedidoLaCotizacion)
					{
						if ($cotizacion->pasarCotizacionAPedido($cotizacion->getIdCotizacion()))
						{
							...
							setTimeout(function () { app.verificarDisponiblePagoPedido();}, 1000);
...
//                 and inventariosucursal.existencia > 0 ")); return $r;
	...
	function pagarConReciboDinero($idPedido, $abonoMonto)
	{
		$r = new xajaxResponse();
