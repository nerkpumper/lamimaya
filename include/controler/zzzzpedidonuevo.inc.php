		...
		for($i = 0 ; $i <count($productos->lstProductos) ; $i++)
		{
			...
			$strListadoProductos .= "
					app.productos.push({
						...
						fullDescripcion: '".$productos->lstProductos[$i]->getDescripcion()."',
						fullDescripcionCode: '".$productos->lstProductos[$i]->getFullDescripcion()."',
...
// $r->mostrarAviso("Productos Cargados"); return $r;
	...
	function cargarProductoIndividual($codigo)
	{
		...
		if (($productos->getPrecio1() > 0  && $productos->getTipoPrecio() == "G")  || $productos->getTipoPrecio() == "T" || $productos->getTipoPrecio()=="I")
		{
			for($i = 0 ; $i <count($productos->lstProductos) ; $i++)
			{
				$desc=str_replace(chr(13),'', $productos->lstProductos[$i]->getRolloDescripcion());
				...
				$strListadoProductos .= "
					app.productos.push({
						...
						estado: '" .$productos->lstProductos[$i]->getEstado() . "',
						existenciaEstimada: '".$productos->lstProductos[$i]->getExistenciaToCero()."',
						fullDescripcion: '".$productos->lstProductos[$i]->getDescripcion(false)."'
...
// 					";
			...
			else
			{
				...
				$r->saError("No se encuentra el producto intenta ingresar.");
			}
		...
		else
		{
			...
			{
				$r->script("ocultarMensaje();");
				$r->saError("El Producto que desea ingresar no cuenta con Precio, favor de verificar.");
			}
			else
			{
				// 			$r->ocultarMensaje();
				$r->script("ocultarMensaje();");
				$r->saError($productos->getIdProducto()  . "  No se encuentra el producto intenta ingresar.");
			}
...
// 				$r->saError($strErrores);
				$r->script(" 
							...
							app.levantarPedidoSinChecarStock(\"".$strErrores."\");
