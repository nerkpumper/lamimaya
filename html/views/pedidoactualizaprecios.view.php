...
<!-- <span>{{ getRangosString }}</span> -->
<!-- <span class="blink_me">idCotizacion: {{ idCotizacion }} - {{ imprimirPedido }}</span> -->

<div v-if="pedidoFolio > 0 && !pedidoDesbloqueado" class="row">
	<div class="col-md-3"></div>
	<div class="col-md-6">
		<div class="alert alert-danger text-center">
			<h2>{{ pedidoFolio }}</h2>
			<h4>
				Pedido bloqueado por cambio de precios en los productos 
			</h4>
			<button v-if="isAdministrador" @click.prevent="desbloquearPedido" class="btn btn-primary btn-block">Desbloquear Pedido sin actualizar precios</button>
...
<div v-if="pedidoDesbloqueado " class="row">
	<div class="col-md-3"></div>
	<div class="col-md-6">
									...
									<div class="col-lg-8 col-md-8 col-sm-8 col-xs-12">
            							<div class="form-group">
            								<label class="control-label">Longitud (Metros Lineales)</label> <input
            									...
            									oninput="this.value = this.value.replace(/[^0-9.\t]/g, '').replace(/(\..*)\./g, '$1');"
            									maxlength="9" class="form-control">
            								<div v-show="molCantUnidad > 6.4 || molCantUnidad <= 0" class="col-lg-12 col-md-12 col-sm-6 col-xs-12">
                    							<div class="alert alert-danger">
                    								Este valor debe ser mayor a cero y menor a 6.4
                    							</div>
                    						</div>
            							</div>
            						</div>
...
<div id="secImprimirONuevo" v-show="false">
		...
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			...
			<div class="ibox">
				...
				<div class="ibox-content">
					...
					<div v-if="verificandoDisponibleParaPagarPedido && rdTotalPedido > 0 && rdDisponible > 0"  class="row">
						<hr>
						
						<div v-show="showAbonarAPedido" class="col-md-6 col-md-offset-3">
							<div class="panel panel-default">
								<div class="panel-heading">								
									<h5 class="panel-title">
										<a data-toggle="collapse" data-parent="#accordion" href="#collapseOne">Disponible de Cliente</a>
									</h5>
								</div>
								<div id="collapseOne" class="panel-collapse">
									<div class="panel-body">
										<div class="pull-right">
											<i class="fa fa-credit-card payment-icon-big text-success"></i>
										</div>
										...
										<div class="row">
											...
											<div class="col-md-10">
												...
												<button @click.prevent="omitirAbono" class="btn btn-danger">
												<i
													class="fa fa-list "></i>
														Omitir abono
													</i>
					...
					<div v-show ='rdDisponible == 0 || !showAbonarAPedido'   class="row">
					...
					<table id="tblPedidoShortcotitopedi"
						...
						<tbody>
							<tr v-for="(item, index) in listadoPedido">
								...
								<!-- <td>
								<span v-if="item.tipoPrecio == 'G'">GALVAMEX</span> <span
	...
	<!-- 											<tr v-show="listadoPedido.length > 0 || totalOtrosCargos > 0"> -->
								...
								<tr v-if="idCotizacion > 0">
									<td colspan="2">
										...
										<button class="btn btn-danger" @click.prevent="updateAllToCurrentPrices">Recalcular Total con Precios Actuales</button>
									</td>
								</tr>
							</tbody>
						</table>
						...
						<div class="panel panel-primary">
							...
							<div class="panel-body">
								...
								<button v-show="mostrarBotonGuardar && idCotizacion > 0									
												...
												|| id_usuario_autorizaimpresion > 0)" @click.prevent="preLevantarPedido(true, true, false, true)"
            ...
            <div class="ibox-footer" >
				<!-- <button v-show="mostrarBotonGuardar && idCotizacion > 0 && 
				                  (!(RDDebeActualizarPrecios && haCambiadoPrecioCotizacion == 'SI') 
								       && ( (RDCubreCotizacion && RDTotalAAmparar > 0) 
									   || (!RDCubreCotizacion && RDTotalAAmparar == 0)) 
								  || id_usuario_autorizaimpresion > 0)" @click.prevent="preLevantarPedido(true, true)"
							class="btn btn-primary btn-lg pull-left">
							<i class="fa fa-link"></i> 											
							<span v-if="idCotizacion > 0">Actualizar Cotizaci&oacute;n y convertir en Pedido</span>
				</button> -->

				<!-- <button v-show="RDDebeActualizarPrecios && haCambiadoPrecioCotizacion == 'SI' && ( (RDCubreCotizacion && RDTotalAAmparar > 0) || (!RDCubreCotizacion && RDTotalAAmparar == 0))" class="btn btn-warning btn-lg pull-left" @click.prevent="updateAllToCurrentPrices">Recalcular Total con Precios Actuales</button> -->
...
<div v-show="vistaPedido && !pedidoDesbloqueado" >
	...
	<div class="ibox-content ibox-heading">
		...
		<h3>
			...
			<i class="fa fa-user"></i> {{ clienteSeleccionado }} <small> {{ clienteTipoRangoSeleccionado }}</small>
			<span v-if="idClienteSeleccionado == 137" class="badge badge-info"> CLIENTE MENDEZ, PRECIO ESPECIAL</span>
			<button v-if="false" @click.prevent="seleccionarOtroCliente" class="btn btn-warning pull-right"><i class="fa fa-users"></i> Seleccionar otro Cliente</button>
	...
	<div class="ibox-content p-xl" id="tablero">
		...
		<div v-show="false && mostrarTiposProducto">
							...
							<table id="tblPedidoShort"
								...
								<thead>
									<tr>
										...
										<th data-sort-ignore="true" data-hide="phone,tablet">Unidad</th>
										<!-- <th data-sort-ignore="true" data-hide="phone,tablet">Tpo Precio</th> -->
										<th data-sort-ignore="true" data-hide="phone,tablet">$/ML Almacenado</th>
										<th data-sort-ignore="true">Subtotal Almacenado</th>
...
<!-- 											Inventario Sucursal -->
											<div v-if="false && item.inventarioSucursal.length > 0 " class="row">
												<br>
                                                <div class="col-lg-12">
                                                    <table class="table table-bordered  margin bottom">
                                                        ...
                                                        <tbody>
                                                            <tr v-for="invsuc in item.inventarioSucursal">                                                                
                                                                <td> {{ invsuc.sucursal }}
                                                                    </td>
...
<!--     										</span> <span v-else>-</span> -->
										...
										<td class="text-danger">  
											$ {{ formatNumber(item.precioRenglonOriginal) }}
											<span v-show="item.idProducto == molIdProducto">
												(Moldura)
											</span>
											<span v-show="item.idProducto == molIdMaquila">
										...
										<td class="text-danger"><strong>$ {{ formatNumber(item.totalRenglonOriginal) }}</strong></td>
										...
										<td class="text-success"><strong>$ {{ formatNumber(item.totalRenglon) }}</strong></td>
										...
										<!-- <td> -->
											...
											<!-- 									<div class="btn btn-warning" @click.prevent="showSectionTiposProductos"><i class="fa fa-angle-double-left"></i> Regresar</div> -->
										<!-- </td> -->
	...
	<!-- 							</tfoot> -->
                        ...
                        <div class="row">
							<div  v-show="maxTipoPrecioGalvamexAcryOpa > 1" class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
								<div class="row">
									...
									<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
										<h3>{{ tipoPrecioGalvamexAcryOpa }}</h3>
                        ...
                        <div class="row" >
							<div  v-show="maxTipoPrecioGalvamexMultipanel > 1" class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
								<div class="row">
									...
									<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
										<h3>{{ tipoPrecioGalvamexMultipanel }}</h3>
                        ...
                        <div class="row" >
							<div  v-show="maxTipoPrecioGalvamexGalvateja > 1" class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
								<div class="row">
									...
									<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
										<h3>{{ tipoPrecioGalvamexGalvateja }}</h3>
                        ...
                        <div class="row" >
							<div  v-show="maxTipoPrecioGalvamexRolloKilo > 1" class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
								<div class="row">
									...
									<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
										<h3>{{ tipoPrecioGalvamexRolloKilo }}</h3>
				...
				<!-- 											<tr v-show="porDescuento > 0"> -->
															<tr>
																<td><strong>SUBTOTAL :</strong></td>
																<td><h3>${{ formatNumber(subtotalPedidoOriginal) }}</h3></td>
															</tr>
				...
				<!-- 											<tr v-show="listadoPedido.length > 0 || totalOtrosCargos > 0"> -->
															<tr>
																...
																<td><h3>${{ formatNumber(totalOtrosCargosOriginal) }}</h3></td>
															</tr>
															...
															<tr>
																...
																<td><h3 class="font-bold">${{ formatNumber(totalPedidoOriginal) }}</h3></td>
															</tr>
															<!-- <tr v-if="pedidoFolio > 0">
																<td colspan="2">
																	<button class="btn btn-warning " @click.prevent="updateAllToCurrentPrices">Recalcular Total con Precios Actuales</button>
																</td>
															</tr> -->
...
<!-- 											<tr v-show="listadoPedido.length > 0 || totalOtrosCargos > 0"> -->
											...
											<tr>
												<td><strong>TOTAL:</strong></td>
												<td>
													<h1 class="font-bold">${{ formatNumber(totalPedido) }}</h1>
													<h4 v-show="totalPedidoOriginal - totalPedido < 0" class="font-bold text-success">+ ${{ formatNumber(totalPedido - totalPedidoOriginal)  }}</h4>
													<h4 v-show="totalPedidoOriginal - totalPedido > 0" class="font-bold text-danger">- ${{ formatNumber(totalPedidoOriginal - totalPedido)  }}</h4>
												</td>
											</tr>
											<!-- <tr v-if="pedidoFolio > 0">
												<td colspan="2">
    									...
    									<div class="row">
    										<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12" v-show=" selRecogeRecibe == 'OBRA' || selRecogeRecibe == 'ENTREGA' || selRecogeRecibe == 'RECOGE' ">
    											<div class="form-group" v-bind:class="{'has-error': errFechaEntrega}">
													...
													<div v-if="false && selRecogeRecibe == 'RECOGE'" >
    ...
    <!-- 									</div> -->
						...
						<div class="row">
							<div class="col-md-6">
								<!-- <button v-show="mostrarBotonGuardar && idCotizacion > 0" @click.prevent="preLevantarPedido(true, true)"
											class="btn btn-primary btn-lg pull-left">
											<i class="fa fa-link"></i> 											
											<span v-if="idCotizacion > 0">Actualizar Cotizaci&oacute;n y convertir en Pedido</span>
								</button> -->
						...
						<div v-if="idCotizacion == 0" class="row">
							<div class="col-md-12">
								<!-- <button v-show="mostrarBotonGuardar" @click.prevent="preLevantarPedido(false, false)"
									class="btn btn-primary btn-lg pull-right">
									<i class="fa fa-shopping-cart"></i> Levantar Pedido
								</button> -->
								<!-- <button class="btn btn-warning pull-right" @click.prevent="updateAllToCurrentPrices">Recalcular Total con Precios Actuales</button> -->
								<button class="btn btn-danger pull-right" @click.prevent="updateAllToCurrentPrices">Actualizar Totales con Precios Actuales</button>
								<button v-show="false && mostrarBotonGuardar" @click.prevent="preActualizarPedido(false, false)"
									class="btn btn-primary btn-lg pull-right">
									<i class="fa fa-shopping-cart"></i> Actualizar Pedido
								</button>
							</div>
						</div>
