var thisversion = "202101021050";
async function updateAsyncOut(index) {
				 await updateWithPromiseOut(index);				 
			}

function updateWithPromiseOut(index) {
				return new Promise(resolve => {
					setTimeout(() => {
						console.log("inside promise index ", index);
	...
	var app = new Vue({
		...
		data: {
			...
			//tipo levantar pedido
			tipoLevantarPasarAPedido: 'XX', 
			//Valores de esta variable
					// LEVANTAR_PEDIDO
					// CONVERTIR_A_PEDIDO_SIN_PLAN
					// CONVERTIR_A_PEDIDO_CON_PLAN

			//clientedatosfiscales
			idClienteDatosFacturacion: -1,

			//buscar por codigo
			buscarXCodigo: true,
			indexPadreDeComercializado: -1,

			//Cargo por Maniobras para Obra en cubierta
			minimoManiobras: 5000,
			chkSeUsaGrua: false,
			pedidoSoloParaObra: false,
	...
	//		Fin Alta Cliente
		...
		mounted: function () {
			...
			{				
				...
				{		  
					if (param1 == 'loadcotizacion')
					{
						this.idCotizacion = param2;		
						// alert("cargaremos coti " + param2);
						this.LoadCotizacion(this.idCotizacion);
					}
				}
			}
...
//  this.productos.push({ idProducto: '518', codigo: '', isMoldura: false, longitud: '6.10', mlpieza: 6.10, idTipoProducto: '1', tipoProducto: 'LÁMINA', shortTipoProducto: 'L', idAplicacion: '36', aplicacion: 'T-81', idMaterial: '23', material: 'STABILIT OPALIT', curva: '', idRollo: '1', rolloCodigo: '-- NO APLICA --', rolloIdMaterial: '1', rolloMaterial: '-- NO APLICA --', rolloShortMaterial: ' NA', rolloIdProveedor: '1', rolloProveedor: '-- NO APLICA --', rolloShortProveedor: 'NA', rolloCalibre: '0', rolloPies: '0.00', rolloPesokiloml: 0, rolloDescripcion: ' ', idUnidad: '4', unidad: 'PIEZA', shortUnidad: 'PZA', calibre: '8', descripcion: 'LAMINA T-81 STABILIT OPALIT GRIS DE 8 ONZAS A 6.10MTS', existencia: '71.00', tipoPrecioComision: 'PRECIO', tipoPrecio: 'G', isRango: '1', tipoRango: 'B', isRollo: '0', precio1: '1912.47', precio2: '1879.95', precio3: '1848.42', precio4: '1812.67', preciomendez: '1722.27', estado: 'ACTIVO', existenciaEstimada: '71', fullDescripcion: 'LÁMINA T-81 STABILIT OPALIT CAL 8 6.10 MTS NACIONAL', fullDescripcionCode: ' - LÁMINA T-81 STABILIT OPALIT CAL 8 6.10 MTS NACIONAL', cantidad: 1, lblUnidad: '', cantUnidad: 1, cantUnidadReal: 1, dobleces: '0', precioRenglon: '0', rangoRenglon: '1', totalRenglon: '0', desarrolloI: '0', desarrolloT: '0', dobleces: '0', debug: '', kl: 0, productoCantidadDisponible: true, molPrecioLamina: 0, molMoldurasXLamina: 1, molMoldurasXLaminaTodos: 1, molLaminasCobrar: 1, molLaminasATomar: 1, molCorte: 0, molDobles: 0, molIsScrap: false, molTotalCMScrap: 0, molLongitudinal: 'L', sugerirStock: [], inventarioSucursal: [], idPedidoDetalle: 0 });
		...
		watch: {
			...
			selTipoObra: function(val){
	
				
				this.calculaTotales();
			
			},
			chkSeUsaGrua: function(val){
				this.calculaTotales();
			},
	...
	// //				console.log("Total OC: " + toc);
	// 				this.totalOtrosCargos = toc;
	// 				// this.calculaTotales();
						this.calcularTotalOtrosCargos();
					},
			  deep: true,
			},
	...
	//			this.calculaPrecioMoldura2();
			...
			molIdRollo: function(val){
				...
				for(i=0; i < this.rollosExistencias.length ; i++)
				{
					if (this.rollosExistencias[i].idrollo == val)
					{
						console.log(this.rollosExistencias[i]);
						this.molStrTemp = " -- " + this.rollosExistencias[i].descauto;
						this.molPrecioRollo = this.rollosExistencias[i].precio1;
						this.molIdMaterial = this.rollosExistencias[i].idmaterial;
						this.molDescripcion = this.rollosExistencias[i].descauto;
	...
	//						this.molDividirLamina = 1;
	//					}
						console.log(molMoldurasXLaminas);
						this.molMoldurasXLaminaTodos = this.molMoldurasXLaminas ;
	...
	//	    		 }
		...
		methods:{			
			calcularTotalOtrosCargos: function(){
				var toc = 0;
				var i;
//				console.log("Parece que algo cambió");
				for (i = 0; i < this.otrosCargos.length ; i++)
...
//				console.log("Total OC: " + toc);
	...
	//						console.log(" si se divide precio " + val);
						...
						if (this.molPies == 2)
						{
							...
							this.molPrecioCM =  this.molPrecioRollo / 61 ;
	...
	//			console.log(" Calculamos si agregamos costo a Doblés, en base a si es longitudinal o ancho");
				...
				if (this.molLongitudinal == "L")
				{
					...
					if (this.molCantUnidad > 3.05 && this.molCantUnidad < 6.11)
					{
					...
					if (this.molCantUnidad > 6.10){

												this.molCostoDobles = this.molCostoDobles * 3; 
												this.molCostoDoblesMaquila = this.molCostoDoblesMaquila * 3;
												//this.molCostoCorte = this.molCostoCorte * 2; 
												//this.molCostoCorteMaquila = this.molCostoCorteMaquila * 2; 
												this.molCostoCorte = this.molCostoCorte * 4.5; 
												this.molCostoCorteMaquila = this.molCostoCorteMaquila * 4.5; 
											}
	...
	//		            	mostrarAviso(resp.msg);
			...
			showPantallaPasarAPedido: function(){
				...
				if (this.selRecogeRecibe == "ENTREGA" && 
					...
					this.totalPedido < 5000)
				{
					var cargo = this.otrosCargos[0].cantidad || 0;					
					if (parseFloat(cargo) == 0)
					{
						if (this.idClienteSeleccionado == 1)
						{
							saInfo("Debes ingresar el costo de Flete, ya que el Pedido es inferior a $ 5, 000 pesos.");
						}
						else
						{
							saInfo("Debes ingresar el costo de Flete, ya que es el primer Pedido del Cliente y es inferior a $ 5, 000 pesos.");
						}
						return;
					}
				}
			...
			preLevantarPedido: function(isCotizacion, pasarAPedidoLaCotizacion, previoAPasarACotizacion = false, usarRD = false){
				...
				else
				{
					if (isCotizacion)
					{
							...
							this.totalPedido < 5000)
						{
							...
							if (parseFloat(cargo) == 0)
							{
								if (this.idClienteSeleccionado == 1)
								{
									saInfo("Debes ingresar el costo de Flete, ya que la Cotización es inferior a $ 5, 000 pesos.");
								}
								else
								{
									saInfo("Debes ingresar el costo de Flete, ya que podría ser el primer Pedido del Cliente y es inferior a $ 5, 000 pesos.");
								}
								return;
							}
						}
					...
					{
						texto = "Se Levantará el Pedido con la información indicada.";
					}
				}
				...
				swal({
					...
					// mdlShowWait();
					setTimeout(app.levantarPedido(app.appisCotizacion, app.apppasarAPedidoLaCotizacion, app.previoAPasarACotizacion, app.utilizarReciboDinero), 100);
	...
	//				{
					...
					for (var indexListado = 0 ; indexListado < this.listadoPedido.length ; indexListado ++)
					{
						...
						this.listadoPedidoShort.push({
							...
							precio4: this.listadoPedido[indexListado].precio4,
						...
						xajax_levantarPedido(this.idClienteSeleccionado, 
											...
											isCotizacion,
											this.idCotizacion,
											this.clienteTipoRangoSeleccionado,
											...
											this.RDATomar,
											this.idClienteDatosFacturacion);
	//				}
	//				else
	//				{
	...
	//				}
			...
			setClienteSelected: function(idCliente){
				...
				this.idClienteDatosFacturacion = -1;
				...
				for (i = 0 ; i < this.clientes.length ; i++)
				{
					if (this.clientes[i].id == idCliente)
					{
						...
						if (this.idUsuarioPromotor != _IDUSUARIO && this.idClienteSeleccionado > 1)
						{
							mostrarAviso("Este Cliente es atendido por \"" +this.promotorClienteSeleccionado+ "\". Se recomienda cotizar con los precios del rango 1. Si se requiere mejorarlos, póngase en contacto con el vendedor titular. Gracias");
						}
						
						if (this.clienteTipoRangoSeleccionado == 'PLATINO')
						{
	//						alert("PLATINO");
							rc = 4;
						}
		...
		//							alert("REGULAR");
	                ...
	                //    console.log("tipoPrecioGalvamex : " + this.tipoPrecioGalvamex);
						...
						{
							xajax_cargarNoPedidosByIdCliente(this.idClienteSeleccionado);
						}
				...
				for (i = 0; i < this.listadoPedido.length; i++)
				{
					if (this.listadoPedido[i].idRollo > 1)
					{
						...
						{	
							if (this.selRecogeRecibe == 'ENTREGA')
							{
								this.msgCotizarManiobras = 'SI LA OBRA ES FUERA DE LA ZONA DE CORTESÍA, NO OLVIDE COTIZAR EL FLETE';
							}
	...
	//				console.log("Total ml RolloKilo: " + this.totalMLRolloKilo);
					...
					/*
					|--------------------------------------------------------------------------
					| Inicio Bloque: Calcular Renta de Equipo por pedido Obra en Cubierta, solo KR 18
					|--------------------------------------------------------------------------
					|
					| El acamalado que genera el costo adicional es la KR-18, y esto ocurre cuando la obra 
					| se realiza en Cubierta; este costo adicional es por el uso de una grúa para subir la máquina 
					| a la cubierta y estarla cambiando de lugar; como el costo por día de la grúa es de $5000, este 
					| será lo mínimo que se cobrará por el concepto de Renta de Equipo en la sección OTROS SERVICIOS. 
					| Lo qué haríamos es multiplicar 2 pesos por los metros de la obra y si esta cantidad es menor a 5000, 
					| se tomaría esta cantidad (5000) como el monto en la Renta de Equipo, pero si la cantidad resultante 
					| al multiplica los 2 pesos por los metros totales de la obra es mayor a los 5000, se tomaría el mayor
					| idAplicacion de KR 18 = 28, 6, 35
					*/
					
					
					// console.log("**renglon ", i);
					// console.log("idAplicacion ", this.listadoPedido[i].idAplicacion);
					if (this.listadoPedido[i].idAplicacion == 28 ||
					    this.listadoPedido[i].idAplicacion == 6  ||
						this.listadoPedido[i].idAplicacion == 35)
					{
						if (this.listadoPedido[i].mlpieza > 0)
						{
							this.totalMLKR18 = this.totalMLKR18 + (this.listadoPedido[i].cantidad * this.listadoPedido[i].cantUnidad * this.listadoPedido[i].mlpieza );
						}
						else
						{
							this.totalMLKR18 = this.totalMLKR18 + (this.listadoPedido[i].cantidad * this.listadoPedido[i].cantUnidad);
						}
	
					}
					/*
					|--------------------------------------------------------------------------
					| Fin Bloque: Calcular Renta de Equipo por pedido Obra en Cubierta, solo KR 18
					|--------------------------------------------------------------------------					
					*/
				}

				// console.log("Total ml KR18: " + this.totalMLKR18);
				//selRecogeRecibe = OBRA
				//selTipoObra = CUBIERTA
				var ioc = this.otrosCargos.findIndex(x => x.descripcion == "MANIOBRAS EN CUBIERTA");
				if (ioc >= 0)
				{
					// console.log("id oc maniobras ", ioc);
					var totalManiobras = 0;
					if (this.totalMLKR18 > 0 && this.selRecogeRecibe == "OBRA" && this.selTipoObra == "CUBIERTA" && this.chkSeUsaGrua)
					{	
						// console.log("Poner total para maniobras");
						totalManiobras = this.totalMLKR18 * 2;
						if (totalManiobras < this.minimoManiobras)
						{
							totalManiobras = this.minimoManiobras;
						}
						
						setTimeout(function(){ saInfo("Se ha realizado el Cálculo de MANIOBRAS EN CUBIERTA de manera automática."); }, 500);
						
					}
					
					this.otrosCargos[ioc].cantidad = totalManiobras;
				}
					...
					*/
					...
					else
					{
						...
						else
						{
							if (this.clienteTipoRangoSeleccionado == 'PLATINO')
							{
								if (this.maxTipoPrecioGalvamex < 4) this.maxTipoPrecioGalvamex = 4;
								if (this.maxTipoPrecioGalvamexMultipanel < 4) this.maxTipoPrecioGalvamexMultipanel = 4; 
								if (this.maxTipoPrecioGalvamexGalvateja < 4) this.maxTipoPrecioGalvamexGalvateja = 4; 
								if (this.maxTipoPrecioGalvamexAcryOpa < 4) this.maxTipoPrecioGalvamexAcryOpa = 4; 
								if (this.maxTipoPrecioGalvamexRolloKilo < 4) this.maxTipoPrecioGalvamexRolloKilo = 4; 
				...
				//se calcula el total de Otros Cargos
				this.calcularTotalOtrosCargos();
				...
				for (i = 0; i < this.listadoPedido.length ; i++)
				{
					...
					if (this.listadoPedido[i].isRango == "1" && this.listadoPedido[i].tipoRango == "A")
					{
						...
						else if (this.tipoPrecioGalvamex == 3)
						{
						...
						{
							this.listadoPedido[i].rangoRenglon = this.comisionR4;
							this.listadoPedido[i].tipoPrecioComision = "RANGO4";
						}
	
	
						if (this.tipoPrecioGalvamex == 4)
						{
							// console.log("Rango Normal: tipo 4");
								
							if (this.listadoPedido[i].mlpieza > 0)
							{
								this.listadoPedido[i].precioRenglon = parseFloat(this.listadoPedido[i].precio4) + (parseFloat(this.maxDescuentoIndividual) * parseFloat(this.listadoPedido[i].mlpieza));
							}
							else
							{
								this.listadoPedido[i].precioRenglon = parseFloat(this.listadoPedido[i].precio4) + parseFloat(this.maxDescuentoIndividual);
							}
						}
						else if (this.tipoPrecioGalvamex == 3)
						{							
					...
					else if(this.listadoPedido[i].isRango == "1" && this.listadoPedido[i].tipoRango == "B")
					{
						...
						else if (this.tipoPrecioGalvamexAcryOpa == 3)
						{
							this.listadoPedido[i].rangoRenglon = this.comisionR3;
							this.listadoPedido[i].tipoPrecioComision = "RANGO3";
						}
						...
						if (this.tipoPrecioGalvamexAcryOpa == 4)
						{
							// console.log("AcryOpa 4 ");
							if (this.listadoPedido[i].mlpieza > 0)
							{
								this.listadoPedido[i].precioRenglon = parseFloat(this.listadoPedido[i].precio4) + (parseFloat(this.maxDescuentoIndividual) * parseFloat(this.listadoPedido[i].mlpieza));
							}
	...
	//					 Manejo de rangos normales tipo C MULTIPANEL
						...
						if (this.tipoPrecioGalvamexMultipanel == 4)
						{
							//  console.log("MULTIPANEL 4");	//							
							if (this.listadoPedido[i].mlpieza > 0)
							{								
								this.listadoPedido[i].precioRenglon = parseFloat(this.listadoPedido[i].precio4) + (parseFloat(this.maxDescuentoIndividual) * parseFloat(this.listadoPedido[i].mlpieza));
							}
						...
						else if (this.tipoPrecioGalvamexMultipanel == 3)
						{
	...
	//					 console.log("estamos en el rango tipo D");
						...
						else if (this.tipoPrecioGalvamexGalvateja == 3)
						{
							this.listadoPedido[i].rangoRenglon = this.comisionR3;
							this.listadoPedido[i].tipoPrecioComision = "RANGO3";
						}
						...
						if (this.tipoPrecioGalvamexGalvateja == 4)
						{
	//						 console.log("Galvateja 4 ");
	
							this.listadoPedido[i].precioRenglon = parseFloat(this.listadoPedido[i].precio4);
						}
						else if (this.tipoPrecioGalvamexGalvateja == 3)
						{
	...
	//					 console.log("estamos en el rango tipo R");
						...
						else if (this.tipoPrecioGalvamexRolloKilo == 3)
						{
							this.listadoPedido[i].rangoRenglon = this.comisionR3;
							this.listadoPedido[i].tipoPrecioComision = "RANGO3";
						}
						...
						if (this.tipoPrecioGalvamexRolloKilo == 4)
						{
	//						 console.log("RolloKilo 4 ");
							this.listadoPedido[i].precioRenglon = parseFloat(this.listadoPedido[i].precio4);
						}
						else if (this.tipoPrecioGalvamexRolloKilo == 3)
						{
	...
	prepararProducto: function(indexPadreDeComercializado = -1, cargarDesdeAjax = true, idProducto = 0){
				...
				if (!isNaN(this.productoAEnlistar))
				{
					...
					if (this.pedidoSoloParaObra)
					{
						...
						{
							saInfo("En Pedidos para Obra, solo puede agregar a la lista Productos que se fabricarán a partir de un Rollo.");
							return;
						}
					}
			...
			cargarRollos: function(){
				// console.log("cargar lista Rollos");
	...
	//			var moldurasSueltas3Pies = 0;
				...
				this.molXLaminas2Pies =   Math.trunc( 91.44 / this.molDesarrolloV2);
				...
				this.moldurasSueltas2Pies = (this.molCantidad % this.molXLaminas2Pies);
				...
				this.molXLaminas2PiesAUsarCompletas = Math.ceil(this.molCantidad / this.molXLaminas2Pies) ;
				...
				this.molXLaminas2PiesAUsar = Math.trunc(this.molCantidad / this.molXLaminas3Pies) ;
	...
	//			molXLaminas4PiesAUsarCompletas: 0,
				...
				if (this.moldurasSueltas2Pies > 0)
				{
	//				this.molSobrante3Pies = (91.44 - (this.moldurasSueltas3Pies * this.molDesarrolloV2));
					this.molSobrante3Pies = ( (61 - (this.molXLaminas2Pies * this.molDesarrolloV2)) * this.molXLaminas2PiesAUsar) + (61 - (this.moldurasSueltas2Pies * this.molDesarrolloV2));
					aUsarPSPP = this.moldurasSueltas2Pies * this.molDesarrolloV2;
	...
	//				A usar Calcular a Pies fin
					...
					// if ( (61 - aUsarPSPP) < 62)
					// {
					this.molSobrante2PiesPSPP = ( (61 - (this.molXLaminas2Pies * this.molDesarrolloV2)) * this.molXLaminas2PiesAUsar) + (61 - (this.moldurasSueltas2Pies * this.molDesarrolloV2));	
					// }
					// else
					// {
				...
				else
				{
					...
					this.molSobrante2Pies = ( (61 - (this.molXLaminas2Pies * this.molDesarrolloV2)) * this.molXLaminas2PiesAUsar);
					this.molSobrante2PiesPSPP = this.molSobrante2Pies;  
				}
	...
	//				this.molSobrante3Pies = (91.44 - (this.molXLaminas3Pies * this.molDesarrolloV2));
			...
			agregarMolduraAPedidoV2: function(usemodal = true)
			{
				...
				if (parseInt(this.molPies) == 2 && parseFloat(this.molDesarrolloV2) >= 61)
				{
					seguir = false;
					this.molError = this.molError + "<br>" + "No puede ingresar mayor o igual a 61 cm, ya que esta manejando 2 pies en el rollo.";
				}
				...
				if (this.molCantUnidad > 8.1 )
				{
					seguir = false;
					this.molError = this.molError + "<br>" + "Metros Lineales debe ser menor o igual a 8.1.";
				}
	...
	//		},
			updateMolduraV2: function(index, usemodal = true){
				...
				setTimeout(function(){ 
					// console.log("ponemos desarrollo " + app.listadoPedido[index].desarrolloT); 
					app.molDesarrolloV2 = app.listadoPedido[index].desarrolloT; }, 100);
				
				setTimeout(function(){ 
					// console.log("ponemos dobleces " + app.listadoPedido[index].dobleces);  
					app.molDoblecesV2 = app.listadoPedido[index].dobleces; } , 150);
	
				setTimeout(function(){ 
					// console.log("ponemos molCalibreFiltroRollo " + app.listadoPedido[index].calibre);  
					app.molCalibreFiltroRollo = app.listadoPedido[index].calibre; } , 175);
	
				setTimeout(function(){ 
					// console.log("ponemos molMaterialFiltroRollo " + app.listadoPedido[index].idMaterial);  
					app.molMaterialFiltroRollo = app.listadoPedido[index].idMaterial; } , 190);
	
				setTimeout(function(){ 
					// console.log("ponemos molCalibreV2 " + app.listadoPedido[index].calibre);  
					app.molCalibreV2 = app.listadoPedido[index].calibre; } , 175);
				
				setTimeout(function(){ 
					// console.log("ponemos molMaterialFiltroRollo " + app.listadoPedido[index].idMaterial);  
					app.molIdMaterialV2 = app.listadoPedido[index].idMaterial; } , 190);
				...
				setTimeout(function(){ 
					// console.log("ponemor rollo " + app.listadoPedido[index].idRollo); 
					app.molIdRolloV2 = app.listadoPedido[index].idRollo;}, 200);
				
				setTimeout(function(){ 
					// console.log("ponemos isscrap " + app.listadoPedido[index].molIsScrap); 
					app.molIsScrap = app.listadoPedido[index].molIsScrap;}, 250);
				
				setTimeout(function(){ 
					// console.log("ponemos totalcmscrap " + app.listadoPedido[index].molTotalCMScrap); 
					app.molTotalCMScrap = app.listadoPedido[index].molTotalCMScrap;}, 300);
	...
	//		    setTimeout(function(){ console.log("ponemor rollo " + app.listadoPedido[index].idRollo); app.molIdRolloV2 = app.listadoPedido[index].idRollo;}, 200);
				setTimeout(function(){ 
					// console.log("ponemor rolloCalibre " + app.listadoPedido[index].rolloCalibre); 
					app.molCalibreV2 = app.listadoPedido[index].rolloCalibre;}, 200);
				
				setTimeout(function(){ 
					// console.log("ponemor rolloIdMaterial " + app.listadoPedido[index].rolloIdMaterial); 
					app.molIdMaterialV2 = app.listadoPedido[index].rolloIdMaterial;}, 220);
	...
	//			alert("quitamos modal cargos");
			...
			updateToCurrentPrices: function(index){
				...
				for (indexAux = 0 ; indexAux < this.productosParaFiltro.length ; indexAux ++)
				{
					if (this.productosParaFiltro[indexAux].idProducto === this.listadoPedido[index].idProducto)
					{
						// console.log("Index AUX UPDATETOCURRENTPRICES ", indexAux)
						// console.log(this.productos[indexAux])
						this.listadoPedido[index].precio1 = this.productosParaFiltro[indexAux].precio1;
						this.listadoPedido[index].precio2 = this.productosParaFiltro[indexAux].precio2;
						this.listadoPedido[index].precio3 = this.productosParaFiltro[indexAux].precio3;
						this.listadoPedido[index].precio4 = this.productosParaFiltro[indexAux].precio4;
						this.listadoPedido[index].preciomendez = this.productosParaFiltro[indexAux].preciomendez;
						break;
					}
				}
				...
				if (!item.isMoldura)
				{
					...
					setTimeout (function() { 
						app.updateProductoLista(index, false);
					}, ((index+1)*700));
					setTimeout (function() { 
						app.enlistaProductoDeModal(false); 
						// this.enlistaProductoDeModal(false); 
				...
				else
				{
					if (item.idProducto == this.molIdProducto)
					{
						...
						setTimeout (function() { 
							app.updateMolduraV2(index, false);
						}, ((index+1)*700));
						setTimeout (function() { 
					...
					else if(item.idProducto == this.molIdMaquila)
					{
						...
						//agregarMolduraAPedidoV2
						setTimeout (function() { 
							app.updateMaquilaV2(index, false);
						}, ((index+1)*700));
						setTimeout (function() { 
							app.agregarMolduraAPedidoV2(false); 
							// this.agregarMolduraAPedidoV2(false); 
	...
	// }, 400);
			...
			updateIndividualToCurrentPricesRecursivo: function(index){
				
				if (index >= this.listadoPedido.length)
					return;

				mdlStatusWait("Actualizando (Recursivo) Cotización Renglon " + (index + 1));
				console.log("++Inicio re calcula con nuevos precios row: " + index);
				// this.updateToCurrentPrices(index);
				updateAsyncOut(index);
				console.log("--Fin re calcula con nuevos precios (Recursivo) row: " + index);
	
				// alert("Siguiente")
				this.updateIndividualToCurrentPricesRecursivo(index + 1);
				
			},
			updateAsync: function(index) {
				 this.updateWithPromise(index);
			},
			updateWithPromise: function(index) {
				return new Promise(resolve => {
					setTimeout(() => {
						app.updateToCurrentPrices(index);
					}, 200);
			...
			updateAllToCurrentPricesBack: function(){
				var i = 0;
				// alert("updateAllToCurrentPrices");
				mdlShowWait();
				mdlStatusWait("Actualizando Cotización");
				for (i = 0 ; i < this.listadoPedido.length ; i++)
				{
					setTimeout(function(index){
						...
						// console.log("++Inicio re calcula con nuevos precios (Back) row: " + index);
						app.updateToCurrentPrices(index);
						// console.log("--Fin re calcula con nuevos precios (Back) row: " + index);
	
					}, (i+1)*450, i);
				}
	
				setTimeout (function() { app.RDDebeActualizarPrecios = false; app.RDPreciosActualizados = true; }, 500 + (550 * this.listadoPedido.length));
				mdlStatusWait("Finalizando Cálculos");
				setTimeout (function() { 
					mdlExitWait(); 
					app.calculaTotales(); 
					// console.log("Fin UpdateAllPrecios") ;
				}, 1500 + (550 * this.listadoPedido.length));
			},			
			updateAllToCurrentPrices: function(){
				var i = 0;
				// alert("updateAllToCurrentPrices");
				mdlShowWait();
				mdlStatusWait("Actualizando Cotización");
				...
				// {
					// setTimeout(function(index){
						...
						// console.log("++Inicio re calcula con nuevos precios row: " + index);
						// app.updateToCurrentPrices(index);						
						app.updateIndividualToCurrentPricesRecursivo(0);
	...
	// 
				...
				setTimeout (function() { 
					...
					app.calculaTotales(); 
					// console.log("Fin UpdateAllPrecios") 
				}, 1500 + (2100 * this.listadoPedido.length));
			},
			updateAllToCurrentPricesTest: function(index){
				// var i = 0;
				// alert("updateAllToCurrentPrices");
				mdlShowWait();
				mdlStatusWait("Actualizando Cotización");
				...
				// {
					// setTimeout(function(index){
						mdlStatusWait("Actualizando Cotización Renglon " + (index + 1));
						// console.log("++Inicio re calcula con nuevos precios (Test) row: " + index);
						app.updateToCurrentPrices(index);
						// console.log("--Fin re calcula con nuevos precios (Test) row: " + index);
				...
				for (let i = 0; i < this.productosNuevoFiltroComercializados.length; i++) {
					...
					if (index < 0){
						this.productosNuevoFiltroComercializadosAgruped.push({
							...
							fullDescripcion: element.fullDescripcion.replace(' ' + element.longitud + ' ', '').replace(' ' + element.mlpieza + ' ', '').replace('  ', ' ').replace('ML', ' '),
							productos: []
						});
						index = this.productosNuevoFiltroComercializadosAgruped.findIndex(x => x.idAplicacion == element.idAplicacion &&
																					   x.idMaterial == element.idMaterial &&
			...
			levantarOConvertirAPedido: function(tipo){
				this.tipoLevantarPasarAPedido = tipo;
				this.idClienteDatosFacturacion = 0;
				
				if (this.idClienteSeleccionado == 1) //Mostrador
				{
					this.levantarOConvertirAPedidoCall();
				}
				else
				{
					this.$refs.nuevaDireccionExistente.show(this.idClienteSeleccionado, this.clienteSeleccionado); 				
				}
				this.showAbonarAPedido = false;
			...
			levantarOConvertirAPedidoCall: function(){

				switch(this.tipoLevantarPasarAPedido)
				{
					case "LEVANTAR_PEDIDO":
						this.preLevantarPedido(false, false);
						break;
					case "CONVERTIR_A_PEDIDO_SIN_PLAN":
						this.preLevantarPedido(true, true);
						break;
					case "CONVERTIR_A_PEDIDO_CON_PLAN":
						this.preLevantarPedido(true, true, false, true)
						break;
				}
