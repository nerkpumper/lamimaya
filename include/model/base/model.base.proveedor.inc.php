<?php
	...
	class ModeloBaseProveedor extends clsBasicCommon
	{
		#------------------------------------------------------------------------------------------------------#
		#----------------------------------------------Propiedades---------------------------------------------#
		#------------------------------------------------------------------------------------------------------#
		var $_nombreClase="base.ModeloBaseProveedor";
		...
		{
			if($idProveedor==0||$idProveedor==""||!is_numeric($idProveedor)|| (is_string($idProveedor)&&!ctype_digit($idProveedor)))return $this->setError("Tipo de dato incorrecto para idProveedor.");
		...
		protected function Insertar()
		{
			try
			{
				...
				if(!$result)
					return $this->setSystemError("Error en la insercion de registro.","[" . $SQL . "][" . mysqli_error($this->dbLink) . "][ModeloBaseProveedor::Insertar]");
				
				$this->idProveedor=mysqli_insert_id($this->dbLink);
				return true;
			}
			catch (Exception $e)
			{
		...
		protected function Actualizar()
		{
			try
			{
				...
				$result=mysqli_query($this->dbLink,$SQL);
				if(!$result)
					return $this->setSystemError("Error en la actualizacion de registro.","[" . $SQL . "][" . mysqli_error($this->dbLink) . "][ModeloBaseProveedor::Update]");
		...
		public function Borrar()
		{
			...
			try
			{
				...
				WHERE idProveedor=" . mysqli_real_escape_string($this->dbLink,$this->idProveedor);
				$result=mysqli_query($this->dbLink,$SQL);
				if(!$result)
					return $this->setSystemError("Error en el borrado de registro.","[" . $SQL . "][" . mysqli_error($this->dbLink) . "][ModeloBaseProveedor::Borrar]");
			}
			catch (Exception $e)
			{
				return $this->setErrorCatch($e);
		...
		public function getDatos()
		{
			try
			{
				...
				$result=mysqli_query($this->dbLink,$SQL);
				if(!$result)
					return $this->setSystemError("Error en la obtencion de detalles de registro.","[ModeloBaseProveedor::getDatos][" . $SQL . "][" . mysqli_error($this->dbLink) . "]");
