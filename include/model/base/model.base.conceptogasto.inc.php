<?php
	...
	class ModeloBaseConceptogasto extends clsBasicCommon
	{
		#------------------------------------------------------------------------------------------------------#
		#----------------------------------------------Propiedades---------------------------------------------#
		#------------------------------------------------------------------------------------------------------#
		var $_nombreClase="base.ModeloBaseConceptogasto";
		...
		{
			if($idConceptoGasto==0||$idConceptoGasto==""||!is_numeric($idConceptoGasto)|| (is_string($idConceptoGasto)&&!ctype_digit($idConceptoGasto)))return $this->setError("Tipo de dato incorrecto para idConceptoGasto.");
		...
		protected function Insertar()
		{
			try
			{
				...
				if(!$result)
					return $this->setSystemError("Error en la insercion de registro.","[" . $SQL . "][" . mysqli_error($this->dbLink) . "][ModeloBaseConceptogasto::Insertar]");
				
				$this->idConceptoGasto=mysqli_insert_id($this->dbLink);
				return true;
			}
			catch (Exception $e)
			{
		...
		protected function Actualizar()
		{
			try
			{
				$SQL="UPDATE " . $this->__tableName . " SET nombre='" . mysqli_real_escape_string($this->dbLink,$this->nombre) . "',
					...
					WHERE idConceptoGasto=" . $this->idConceptoGasto;
				
				$result=mysqli_query($this->dbLink,$SQL);
				if(!$result)
					return $this->setSystemError("Error en la actualizacion de registro.","[" . $SQL . "][" . mysqli_error($this->dbLink) . "][ModeloBaseConceptogasto::Update]");
		...
		public function Borrar()
		{
			...
			try
			{
				...
				WHERE idConceptoGasto=" . mysqli_real_escape_string($this->dbLink,$this->idConceptoGasto);
				$result=mysqli_query($this->dbLink,$SQL);
				if(!$result)
					return $this->setSystemError("Error en el borrado de registro.","[" . $SQL . "][" . mysqli_error($this->dbLink) . "][ModeloBaseConceptogasto::Borrar]");
			}
			catch (Exception $e)
			{
				return $this->setErrorCatch($e);
		...
		public function getDatos()
		{
			try
			{
				...
				$result=mysqli_query($this->dbLink,$SQL);
				if(!$result)
					return $this->setSystemError("Error en la obtencion de detalles de registro.","[ModeloBaseConceptogasto::getDatos][" . $SQL . "][" . mysqli_error($this->dbLink) . "]");
