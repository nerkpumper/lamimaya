...
// 		{
// 			$r->saError("No se pudo cargar informaci�n del Pedido sldkjfkldsjlfkdsj.");
...
// 		}
		...
		$datosCliente .= "app.saldo = ".$pedido->getPedidoDato("saldo").";";
	...
	function registrarMovimiento($idPedido, $cteIdCliente, $abonoMovimiento, $abonoMonto, $abonoFormaPago, $abonoReferencia, $idSucursal)
	{
		$r = new xajaxResponse();
		$pedido = new ModeloPedido();
		...
		if ($abonoMonto > $pedido->getSaldo() )
		{
			$r->saInfo("No se ha realizado el movimiento, debido a que el Monto que ha indicado, supera el Saldo que tiene el Pedido. Probablemente se ha hecho algún movimiento recientemente. Favor de verificar.");
		...
		$cc->NotificarPasarPorEfectivo($idSucursal);
		if ($abonoMovimiento == "ABONO")
		{
			// $r->mostrarAviso(FOLDERLOGS .$idPedido.'.html');
			ob_start();
