...
class PDF extends PDFNerk
{
	...
	function Header()
	{
	    ...
	    // Logo
	    $this->Image('img/galvalogo.jpeg',5,$yInicial-4,50);
	    // Arial bold 15
...
// 	    {
			...
			$this->Line(25, $yInicial+42, 25, $yInicial+42+135);
			$this->Line(160, $yInicial+42, 160, $yInicial+42+135);
	    	$this->Line(180, $yInicial+42, 180, $yInicial+42+135);
...
// 	    }
	    ...
	    $this->putText(12, "CANT.");
		$this->putText(80,"CONCEPTO");
		$this->putTextRight("PRECIO",40 );
	    $this->putTextRight("IMPORTE", 17);
  		            ...
  		            $desc = $item["proTipoProducto"] . " " .
							...
							$item["proMaterial"] . " " .							
  		  		            ...
  		  		            else
  		  		            {
									...
									if ($item["proLongitud"] != "" AND $item["rolloIdProveedor"] != "1"  )
									{
										$desc .= $item["rolloPies"]." PIES de ".$item["proLongitud"] ." ML";
									}
									else
									{
  		  		            ...
  		  		            if ($item["detIdProducto"] == $this->idProductoMoldura)
  		  		            {
  		  		                ...
  		  		                if ($item["detDesarrollo"] != "0" && $item["detDobleces"] != "0")
  		  		                {
  		  		                    ...
  		  		                    $desc .= " {Des: " .$item["detDesarrollo"]. " - Dbl: ".$item["detDobleces"]."} ".$item["rolloMolduraCodigo"];
  		  		                }
  		  		            ...
  		  		            //   		  		        else
									...
									$detPrecioUnitarioSinIva = $item["detPrecioUnitario"]/1.16 ;
									
									$detTotalSinIva = $item["detTotal"]/1.16;
								$this->putTextRight('$'.number_format($detPrecioUnitarioSinIva,2), 38);
...
//   		$this->putTextRight("1234", 160);
		...
		$this->setCurrentY($yInicial + 182);
  		...
  		$this->putTextRight(number_format(($this->__subtotal/1.16) + ($sumOtrosCargos/1.16), 2), 14.5);
		  $this->nextRow(3);
		  $this->putTextRight(number_format((($this->__subtotal/1.16)*.16)+(($sumOtrosCargos/1.16)*.16), 2), 14.5);
...
$lstOcp = $ocp->getAll("otroscargoscotizacion.*, oc.descripcion, oc.ingreso, oc.solicitar",
                       "inner join otrocargo as oc
                        on oc.idotrocargo = otroscargoscotizacion.idotrocargo",
                       "idcotizacion = " . $idCotizacion);

if ($cotizacion->getPedidoDato("idCliente") == "137")
{
	$pdf->isMDM = true;
}

$pdf->__idUsoCfdi = doubleval($cotizacion->getPedidoDato("idUsoCfdi"));
$pdf->__recogeEntrega = $cotizacion->getPedidoDato("recogeentrega");

$lstSucursales = array(); 
//  $cotizacion->getDataSet("
...
$pdf->__nombre =utf8_decode(mb_strtoupper($cotizacion->getPedidoDato("cteNombre") . " " . $cotizacion->getPedidoDato("cteApellidos")));
$pdf->__fecha = utf8_decode(mb_strtoupper($cotizacion->getPedidoDato("fecha_capturado")));
$pdf->__domicilio = utf8_decode(mb_strtoupper($cotizacion->getPedidoDato("cteDomicilio1") . " " . $cotizacion->getPedidoDato("cteDomicilio2") . " " . $cotizacion->getPedidoDato("cteNumero") . " " . $cotizacion->getPedidoDato("cteColonia")));
$pdf->__ciudad = utf8_decode(mb_strtoupper($cotizacion->getPedidoDato("cteCiudad")));
$pdf->__rfc = utf8_decode(mb_strtoupper($cotizacion->getPedidoDato("cteRfc")));
...
$pdf->__subtotal = doubleval($cotizacion->getPedidoDato("subtotal"));
$pdf->__iva = doubleval($cotizacion->getPedidoDato("iva"));
$pdf->__descuento = doubleval($cotizacion->getPedidoDato("descuento"));
$pdf->__total = doubleval($cotizacion->getPedidoDato("total"));

$pdf->__horaRecibe = $cotizacion->getPedidoDato("horaRecibe");
$pdf->__fechaCompromiso = $cotizacion->getPedidoDato("fechaCompromiso");
...
$pdf->_recibePersona = utf8_decode(mb_strtoupper($cotizacion->getPedidoDato("personaEntrega")));
$pdf->_recibeDomicilio = utf8_decode(mb_strtoupper($cotizacion->getPedidoDato("domicilioEntrega")));
$pdf->_recibeNumero = utf8_decode(mb_strtoupper( $cotizacion->getPedidoDato("numeroEntrega")));
$pdf->_recibeColonia = utf8_decode(mb_strtoupper($cotizacion->getPedidoDato("coloniaEntrega")));
$pdf->_recibeCiudad =utf8_decode(mb_strtoupper( $cotizacion->getPedidoDato("ciudadEntrega")));

$pdf->__fechaAbierta = utf8_decode(mb_strtoupper( $cotizacion->getPedidoDato("fechaAbierta")));
$pdf->__pedidoExpress = utf8_decode(mb_strtoupper( $cotizacion->getPedidoDato("pedidoExpress")));

$pdf->_observacionPedido = $cotizacion->getPedidoDato("observacionCaptura");

$pdf->__rangoCliente = $cotizacion->getPedidoDato("rangoCliente");
...
$pdf->_totalHojas = ceil(count($cotizacion->__rsCotizacionWDetalle) / $pdf->_rowsByHoja);


$pdf->printDatosPedido($cotizacion->__rsCotizacionWDetalle, $lstOcp, 0);

for($h = 1 ; $h < $pdf->_totalHojas ; $h++)
{
    $pdf->AddPage();
    $pdf->printDatosPedido($cotizacion->__rsCotizacionWDetalle, $lstOcp, $h);
