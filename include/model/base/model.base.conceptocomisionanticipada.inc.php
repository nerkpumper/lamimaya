<?php
	...
	class ModeloBaseConceptocomisionanticipada extends clsBasicCommon
	{
		#------------------------------------------------------------------------------------------------------#
		#----------------------------------------------Propiedades---------------------------------------------#
		#------------------------------------------------------------------------------------------------------#
		var $_nombreClase="base.ModeloBaseConceptocomisionanticipada";

		
		var $idConceptoComisionAnticipada=0;
		var $nombre='0';
		var $estado='ACTIVO';
		...
		var $__s=array("idConceptoComisionAnticipada",
                       "nombre",
                       "estado",
                       "fecha_creacion",
                       "idUsuarioCrea",
		...
		public function setIdConceptoComisionAnticipada($idConceptoComisionAnticipada)
		{
			if($idConceptoComisionAnticipada==0||$idConceptoComisionAnticipada==""||!is_numeric($idConceptoComisionAnticipada)|| (is_string($idConceptoComisionAnticipada)&&!ctype_digit($idConceptoComisionAnticipada)))return $this->setError("Tipo de dato incorrecto para idConceptoComisionAnticipada.");
			$this->idConceptoComisionAnticipada=$idConceptoComisionAnticipada;
			$this->getDatos();
		}
		...
		public function getIdConceptoComisionAnticipada()
		{
			return $this->idConceptoComisionAnticipada;
		}
		...
		protected function limpiarPropiedades()
		{
			...
			$this->idConceptoComisionAnticipada=0;
			$this->nombre='0';
			$this->estado='ACTIVO';
		...
		protected function Insertar()
		{
			try
			{
				...
				if(!$result)
					return $this->setSystemError("Error en la insercion de registro.","[" . $SQL . "][" . mysqli_error($this->dbLink) . "][ModeloBaseConceptocomisionanticipada::Insertar]");
				
				$this->idConceptoComisionAnticipada=mysqli_insert_id($this->dbLink);
				return true;
			}
			catch (Exception $e)
			{
		...
		protected function Actualizar()
		{
			try
			{
				$SQL="UPDATE " . $this->__tableName . " SET nombre='" . mysqli_real_escape_string($this->dbLink,$this->nombre) . "',
					...
					WHERE idConceptoComisionAnticipada=" . $this->idConceptoComisionAnticipada;
				
				$result=mysqli_query($this->dbLink,$SQL);
				if(!$result)
					return $this->setSystemError("Error en la actualizacion de registro.","[" . $SQL . "][" . mysqli_error($this->dbLink) . "][ModeloBaseConceptocomisionanticipada::Update]");
		...
		public function Borrar()
		{
			...
			try
			{
				...
				WHERE idConceptoComisionAnticipada=" . mysqli_real_escape_string($this->dbLink,$this->idConceptoComisionAnticipada);
				$result=mysqli_query($this->dbLink,$SQL);
				if(!$result)
					return $this->setSystemError("Error en el borrado de registro.","[" . $SQL . "][" . mysqli_error($this->dbLink) . "][ModeloBaseConceptocomisionanticipada::Borrar]");
			}
			catch (Exception $e)
			{
				return $this->setErrorCatch($e);
		...
		public function getDatos()
		{
			try
			{
				...
				$result=mysqli_query($this->dbLink,$SQL);
				if(!$result)
					return $this->setSystemError("Error en la obtencion de detalles de registro.","[ModeloBaseConceptocomisionanticipada::getDatos][" . $SQL . "][" . mysqli_error($this->dbLink) . "]");
		...
		public function Guardar()
		{
			...
			if($this->getError())
				return false;
			if($this->idConceptoComisionAnticipada==0)
				$this->Insertar();
			else
				$this->Actualizar();
			if($this->getError())
