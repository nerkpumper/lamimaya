...
// 	var $yInicial = 153.5;
	...
	function Header()
	{
		$this->printHeader($this->yInicial);
		$this->printHeader($this->yInicial + 148.5);
	}
	
	private function printHeader($yInicial)
	{		
		// Logo
		$this->Image('img/galvalogo.jpeg',5,$yInicial-11,50);
		// Arial bold 15
		$this->SetFont('Arial','B',11);
		 
// 		$this->setCurrentY($this->GetPageHeight()/2);
// 		$this->putText(10, $this->GetPageHeight()/2);
		...
		$this->nextRow();
		$this->putTextCenter("BLVD. JUAN JOSE TORRES LANDA No. 2507", -10);
		...
		$this->RoundedRect(10, $yInicial + 40, 192, 7, 3, '12', 'DF');
		$this->RoundedRect(10, $yInicial + 40, 192, 60, 3, '1234', 'D');
		$this->Line(28, $yInicial+40, 28, $yInicial+40+60);
		$this->Line(182, $yInicial+40, 182, $yInicial+40+60);
		$this->SetFont('Arial','B',10);
		 
		$this->setCurrentY($yInicial + 44);
		$this->putText(11, "PARTIDA");
		$this->putTextCenter("CONCEPTO");
		$this->putTextRight("TOTAL", 12);
		 
		 
		$this->RoundedRect(155, $yInicial + 101, 27, 22, 3, '14', 'DF');
		$this->RoundedRect(182, $yInicial + 101, 20, 22, 3, '23', 'D');
		$this->Line(155, $yInicial + 108, 202, $yInicial+108);
		$this->Line(155, $yInicial + 115, 202, $yInicial+115);
		 
		$this->SetFont('Arial','',10);
		 
		$this->setCurrentY($yInicial + 106);
		...
		$this->nextRow(-3);
		$this->putText(25, "________________________                         ________________________");
...
// 	    //$this->Cell(0,10,'Page '.$this->PageNo().'/{nb}',0,0,'C');
	...
	{
		$this->imprimirDatosDePedido($this->yInicial - 2, $rsDetalle);
		$this->imprimirDatosDePedido($this->yInicial + 146.5, $rsDetalle);
  		
	}
	
	private function imprimirDatosDePedido($yInicial, $rsDetalle)
	{
// 		$yInicial = $this->yInicial - 2;
		...
		if ($this->__isPedido)
		{
			$this->Line(152, $this->currentY - 3, 159, $this->currentY + 4);
			$this->Line(152, $this->currentY + 4, 159, $this->currentY - 3);
		}
		else
		{
			$this->Line(192, $this->currentY - 3, 199, $this->currentY + 4);
			$this->Line(192, $this->currentY + 4, 199, $this->currentY - 3);
		}
		...
		$this->putTextRight(number_format($this->__subtotal, 2), 8.5);
		$this->nextRow(3);
		$this->putTextRight(number_format($this->__iva, 2), 8.5);
		
		$this->SetFont('Arial','B',10);
		
		$this->nextRow(3);
		$this->putTextRight(number_format($this->__total, 2), 8.5);
		...
		foreach ($rsDetalle as $item)
		{
			...
			$desc = $item["proCodigo"] . " " .
					...
					$this->putTextRight(number_format($item["detTotal"],2), 8.5);
					$this->nextRow();
		}
